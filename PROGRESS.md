...
| 2.5 | **service_account切替（cloud_run_sa）** | IaC | ✅ 完了 | terraform apply + 両サービスともcloud-run-sa確認済み（2026-02-19） |
| 2.6 | **requirements.txt 全固定** | 品質 | ✅ 完了 | google-cloud-storage==2.19.0 に固定（2026-02-19） |
| 2.7 | **Phase 4C: デッドルート削除** | 保守 | ✅ 完了 | 4デッドルート削除 2907→2236行（671行削減、2026-02-19） |
| 2.8 | **Zoom機能全面強化 Phase Z1+Z2（7機能）** | 新機能 | ✅ 完了 | PR #645 マージ済み（2026-02-20） |
| 3 | **P7（DB型統一）** | 見送り | ⏸ LOW urgency / HIGH risk | 将来のメジャーアップデート時に対応 |
| 4 | **Google Meet連携** | 新機能 | ✅ 完了 | Drive録画→Whisper文字起こし→議事録生成。PR #638（2026-02-19）。ENABLE_GOOGLE_MEET_MINUTES=trueで有効化 |
| 5 | **LangGraph導入** | 戦略改善 | 未着手 | Brain判断フローの可視化・分割 |
| 6 | **Phase 4B: 外部連携API** | 新機能 | 未着手 | 公開API |

### ✅ 2026-02-20 Zoom機能全面強化完了（PR #645）

**PR #645**: feat(zoom): Zoom機能全面強化 Phase Z1+Z2（7機能）マージ済み

| フェーズ | 機能 | 効果 |
|---------|------|------|
| Z1 ① | transcript_completedトリガー対応 | 議事録生成の待ち時間3.5分→0分 |
| Z1 ② | 管理ダッシュボードZoom設定ページ | 設定変更の工数ゼロ化 |
| Z1 ⑤ | タスク期限1週間固定+deadline_hintパース | 期限付きタスクの自動設定 |
| Z2 ③ | 複数Zoomアカウント対応 | 会議ルーム自動ルーティング |
| Z2 ④ | カレンダー検索ウィンドウ拡大±30m→±60m | 開始時刻ズレによる検索失敗ゼロ化 |
| Z2 ⑥ | 議事録からソウルくん自動学習 | 「あの会議で何決めた？」に答えられる |
| Z2 ⑦ | 送信失敗→管理者DM通知 | 無言の失敗を即座に検知 |
...
**目的:** lib/brain/全体にmypy型チェックを段階的に適用

**修正ファイル（18件）:**
- Phase 1: core.py, guardian.py, ceo_learning.py
- Phase 2: validation.py, learning.py, chain_of_thought.py
- Phase 3: tool_converter.py, integration.py, decision.py, execution.py, observability.py, llm_brain.py, episodic_memory.py, org_graph.py, understanding.py, ceo_teaching_repository.py, value_authority.py, learning_foundation/__init__.py

**成果:**
- mypyエラー: 39件 → 13件（主要40ファイル）
- 型アノテーション追加: Dict, List, Optional等
- LearningInsightデータクラス拡張（欠落フィールド追加）

**残り13件の性質:** `Optional[str]`を`str`に渡す問題（dataclass設計の課題、将来対応）
...
**問題**: GoalInfo/TaskInfo/PersonInfoが4箇所にバラバラに定義されていた
...
営業部:「名前と電話番号を書くカードね」
経理部:「名前と請求先住所を書くカードね」
サポート:「名前と問い合わせ履歴を書くカードね」

→ 全部「顧客カード」だけど中身がバラバラ！
→ 営業が作ったカードを経理に渡すと「これ違う形式だよ」とエラーになる

【解決】
総務:「顧客カードの正式フォーマットはこれです！」
→ 全部署がその1つの定義を使う
```
...
| 変更前 | 変更後 |
|--------|--------|
| `context_builder.py` にGoalInfo定義 | `models.py`からimport |
| `memory_access.py` にGoalInfo定義 | `models.py`からimport |
| `goal_expert.py` にGoalInfo定義 | `models.py`からimport |
| `models.py` にGoalInfo定義（簡素版） | **SoT: 全フィールド統合版** |

同様にTaskInfo、PersonInfoも統一。

**テスト結果:** 全231件パス

---

### 🔧 本番障害修正 - 完全調査レポート（2026-01-31 23:30）

**症状**: ソウルくんが「処理中に問題が発生したウル」エラーを返す

**カズさんからの報告:**
```
菊地: 俺の目標だして
ソウル: 処理中に問題が発生したウル...もう一度試してほしいウル♪

菊地: チャットワークのタスクだして
ソウル: 処理中に問題が発生したウル...もう一度試してほしいウル♪

菊地: 管理部のチャットグループに「テスト」って発信して
ソウル: 処理中に問題が発生したウル...もう一度試してほしいウル♪
```
...
ソウルくんの脳は、ユーザーの情報（目標やタスクなど）を「箱」から取り出して使います。
...
脳:「この箱、袋みたいに探せば中身が出てくるはず...」
箱:「私は箱だよ！ラベル読んでくれないと中身出せないよ！」
→ エラー
```
...
```
脳:「確信度は 0.85 だな。この情報をDBに保存しよう」
DB:「え、これ数字じゃなくて複雑なオブジェクトだよ？保存できないよ！」
→ エラー
```

**問題3: （追加調査で発見）別の場所でも同じ問題があった**

```
脳:「この操作を実行する判断を作ろう」
判断システム:「確信度は数字で教えてくれないと困る！」
脳:「え、オブジェクトそのまま渡しちゃった...」
→ 潜在的なエラー
```
...
| 問題 | 修正前 | 修正後 |
|------|--------|--------|
| 箱の開け方 | `箱.探す("ラベル")` | `箱.ラベル` |
| 確信度の保存 | オブジェクト全体 | `オブジェクト.数値` |
| 判断システム | オブジェクト全体 | `オブジェクト.数値` |
| デバッグ情報 | オブジェクト全体 | `オブジェクト.辞書化()` |
...
### エラーログ
```
Error getting active goals: 'GoalInfo' object has no attribute 'get'
TypeError: Object of type ConfidenceScores is not JSON serializable
```
...
| # | 重要度 | ファイル:行 | 問題 | 修正内容 |
|---|--------|------------|------|----------|
| 1 | **重大** | `context_builder.py:593` | GoalInfoオブジェクトを辞書として扱っていた | `g.get("id")` → `g.id` |
| 2 | **重大** | `core.py:1529` | LLMPendingAction.confidenceにConfidenceScoresを渡していた | `.overall`で数値化 |
| 3 | **重大** | `core.py:1597` | DecisionResult.confidenceにConfidenceScoresを渡していた | `.overall`で数値化 |
| 4 | 中 | `core.py:1499` | debug_info内のconfidenceがオブジェクトのまま | `.to_dict()`で辞書化 |
| 5 | 中 | `core.py:1547` | debug_info内のconfidenceがオブジェクトのまま | `.to_dict()`で辞書化 |
| 6 | 中 | `core.py:1576` | debug_info内のconfidenceがオブジェクトのまま | `.to_dict()`で辞書化 |
| 7 | 中 | `core.py:1634` | debug_info内のconfidenceがオブジェクトのまま | `.to_dict()`で辞書化 |

### 根本原因（設計問題）

**同じ名前のクラスが複数ファイルで異なる定義になっている:**

| クラス名 | 定義場所 |
|---------|---------|
| GoalInfo | `memory_access.py`, `context_builder.py`, `models.py`, `agents/goal_expert.py` |
| TaskInfo | `memory_access.py`, `context_builder.py`, `models.py`, `agents/task_expert.py` |
| PersonInfo | `memory_access.py`, `context_builder.py`, `models.py`, `agents/organization_expert.py` |
...
| ファイル | 理由 |
|---------|------|
| `core.py:980-1023` | `hasattr()`チェックを使用した防御的コーディング |
| `guardian_layer.py:399` | `_check_confidence()`は`ConfidenceScores`型を正しく期待 |
| `context_builder.py:621-627` | `hasattr()`チェックを使用した防御的コーディング |

### 修正コミット

1. **30a1063**: 初回修正（GoalInfo, LLMPendingAction.confidence）
2. **f81ffc6**: 追加修正（DecisionResult, debug_info全箇所）

### デプロイ履歴

| 時刻 | Revision | 内容 |
|------|----------|------|
| 22:40 JST | chatwork-webhook-00309-gqm | 初回修正デプロイ |
| 23:40 JST | chatwork-webhook-00310-z2j | 追加修正デプロイ ✅ 完了 |

### PR
- https://github.com/soulsyncs/soul-kun/pull/368

---

## 📋 今後の対策（技術的負債）

1. **クラス定義の統一**: GoalInfo, TaskInfo, PersonInfo を1箇所で定義
2. **型チェックの強化**: mypyなどの静的型チェッカーの導入検討
3. **シリアライズテストの追加**: dataclassオブジェクトがJSONに変換できることを自動テスト
...
**main.py 分割 Phase 5-10 完了 + 本番デプロイ完了** ✅✅
- 最終行数: **5,505行**（8,255行から**2,752行削減、33%削減**）
- **本番デプロイ完了**: 2026-01-30T00:23:48Z
- リビジョン: chatwork-webhook-00288-tef
- 設計原則「全入力は脳を通る」の準拠達成
...
| 8 | 未使用関数削除 | -227行 |
| 9 | 未使用import削除 | -16行 |
| 10 | Brain移行+ハンドラー抽出 | -1,729行 |
| **累計** | | **-2,752行** |

**新規ファイル:**
- `lib/brain/handler_wrappers.py`（1,670行）- 21個のBrainハンドラー

**本番環境:**
- ✅ `USE_BRAIN_ARCHITECTURE=true` 設定済み
- ✅ lib/brain loaded (v10.48.1), enabled=True
- ✅ 起動ログ確認（エラーなし）
- ✅ **Hotfix v10.48.1/v10.48.2 デプロイ完了** (2026-01-30 10:17 JST)

**🔧 Hotfix v10.48.1/v10.48.2 (2026-01-30 10:17 JST)**
| PR | 内容 | 問題 |
|----|------|------|
| #343 | state_manager.py async/sync修正 | `'Connection' object does not support the asynchronous context manager protocol` |
| #344 | addon_manager.py text()追加 | `Not an executable object` (SQLAlchemy 2.0互換性) |

- リビジョン: chatwork-webhook-00279-pvp

---

### 🔥 確認フロー問題の根本分析 (2026-01-30 12:20 JST)

**症状**: 一般会話（「うん」「今日どう？」等）に対して「一般会話でいいですか？」と確認が発動

**Hotfix履歴 (v10.48.6〜v10.48.11):**
| Version | 修正内容 | 結果 |
|---------|---------|------|
| v10.48.6 | merged_paramsにmessage追加 | 部分改善 |
| v10.48.7 | handler_wrappersでparams.messageフォールバック | 部分改善 |
| v10.48.8 | execution.pyリトライ条件修正 | 部分改善 |
| v10.48.9 | リスクベース閾値導入(LOW=0.50) | 部分改善 |
| v10.48.10 | RISK_LEVELSにgeneral_conversation追加 | 部分改善 |
| v10.48.11 | general_conversation確認スキップ | **一時対処** |

**根本原因の特定:**

```python
# decision.py 422-431行目
if not candidates:
    return DecisionResult(
        action="general_conversation",
        confidence=0.3,  # ← ここが問題の元凶
        ...
    )
```

**設計との矛盾:**
- 設計書: 「どの機能にもマッチしない→一般会話フォールバック」
- 本来の意味: 「一般会話として処理する」への確信度は**高い**べき
- 現実装: `confidence=0.3`（ハードコード） → 閾値(0.50)未満 → 確認発動

**v10.48.11の評価:**
| 観点 | 評価 |
|------|------|
| 即効性 | ✅ 問題は解消 |
| 設計準拠 | ⚠️ 対症療法的（ハードコード例外） |
| 拡張性 | ⚠️ 他のアクションで同様の問題が起きたら個別対応必要 |
...
if not candidates:
    return DecisionResult(
        action="general_conversation",
        confidence=0.7,  # 0.3→0.7に変更
        ...
    )
```

**理由**: 「どの機能にも該当しない」という判断自体は確実。「一般会話として処理する」というルーティングへの確信は高い。
...
**次のステップ: Phase 11計画（大関数抽出）**

| Phase | 内容 | 削減見込み |
|-------|------|-----------|
| 11-A | get_ai_response → lib/brain/response.py | -315行 |
| 11-B | タスク関連 → handlers/task_handler.py | -718行 |
| 11-C | メモリ関連 → handlers/memory_handler.py | -278行 |
| 11-D | ユーティリティ → utils/各モジュール | -699行 |
| **合計** | | **-2,010行** |

Phase 11完了後: 5,505行 → ~3,500行

**優先度:**
1. ⏳ 本番動作確認待ち（ChatWorkでメンションしてテスト）
2. Phase 11-Aの実施（get_ai_response移動）
...
| **G2-G4** | **画像/リサーチ/動画** | ✅ 完了 | 2026-01-28 | G2画像47テスト、G3リサーチ49テスト、G4 Sheets/Slides 56テスト |
...
- **3AI設計品質改善プランP8/P15を実装** ✅ **PR #614 マージ完了（v11.2.1）**
  - **概要**: P8（フォールバック送信禁止）・P15（ログのorg_idマスキング）
  - **変更**: 9ファイル（lib/ 3コピー同期済み）
  - **P8**: `lib/brain/proactive.py` — 脳（Brain）が使えない時の定型文直送を廃止。Silent Fail（送らずに終了）に変更。CLAUDE.md §1「全出力は脳を通る」に完全準拠。dead codeの `_generate_message` 呼び出しも消滅（メソッドはコメントで明示）
  - **P15**: org_id（UUID）をログに生値で出力していた5箇所を先頭8文字のみに変更（CLAUDE.md §9-3 PII保護）
  - **追加調査**: P17（Langfuse）= コード実装済み・本番環境変数設定のみで有効化可能。P14（DB接続管理）= pool_size/max_overflow/pool_pre_ping 全て適切に設定済み。どちらもコード修正不要と判断。
  - **3AIレビュー**: brain-reviewer PASS（W1-W4全対応）/ Codex PASS / Gemini PASS
  - **CI結果**: Quality Gate含む全重要チェック SUCCESS
