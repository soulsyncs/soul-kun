...
### DB設計のSoTルール【v10.55追加】

> **原則**: テーブル定義（CREATE TABLE文）は **03_database_design.md がSoT**。
> 他の設計書ではテーブル定義の詳細を記載せず、03章への参照のみとする。

| 設計要素 | SoT（正） | 参照のみ | 備考 |
|---------|----------|---------|------|
| departments テーブル | 03章 5.2.5 | 06章（Phase 3.5） | 06章は手順書、定義は03章 |
| user_departments テーブル | 03章 5.2.5 | 06章（Phase 3.5） | 同上 |
| department_access_scopes テーブル | 03章 5.2.5 | 06章（Phase 3.5） | 同上 |
| department_hierarchies テーブル | 03章 5.2.5 | 06章（Phase 3.5） | 同上 |
...
```
1. 03_database_design.md を更新（SoT）
2. 参照元（06章等）が03章を参照していることを確認
3. マイグレーションファイルは03章の定義をコピー
```

> **禁止事項**: 03章以外の設計書にCREATE TABLE文のコピーを記載しない。
> 理由: 更新時に片方だけ変更されて不整合が発生するリスクを防止。
...
| デプロイプロセス（手順・安全チェック） | deploy.sh（各Function） | 全体 | ✅ | 2026-02-10追加 |
| CI/CD（自動デプロイ） | cloudbuild.yaml, cloudbuild-proactive-monitor.yaml | 全体 | ✅ | 2026-02-10追加 |
| lib/ 3コピー同期 | scripts/sync_lib.sh | 全体 | ✅ | 2026-02-10追加 |
| SQLカラム検証 | scripts/validate_sql_columns.sh | 全体 | ✅ | 2026-02-10追加 |
...
| 3層防御（予防・検出・確認） | CLAUDE.md | セクション13 | ✅ | 2026-02-10追加 |
| Codexレビュー（3パス18項目） | scripts/codex_review_loop.sh | 全体 | ✅ | 2026-02-10追加 |
...
## 10. 戦略的改善計画【2026-02-14追加】

| 設計要素 | SoT（正のファイル） | セクション | 状態 | 備考 |
|---------|-------------------|-----------|------|------|
| 戦略的改善 優先順位合意 | 30章 | 第8章 | ✅ | 3AI合同分析（2026-02-14） |
| Langfuse導入計画 | 30章 | 4.1 | ✅ | 可観測性（優先度1） |
| Cloud Run移行計画 | 30章 | 4.2 | ✅ | インフラ（優先度2） |
| LangGraph導入計画 | 30章 | 4.5 | ✅ | Brain改善（優先度3） |
| Promptfoo導入計画 | 30章 | 5.3 | ✅ | 品質管理（優先度4） |
| Terraform (IaC)計画 | 30章 | 6.2 | ✅ | インフラ管理（優先度5） |
| MCP統合強化計画 | 30章 | 4.3 | ✅ | 拡張性（優先度6） |
| iPhoneアプリ計画 | 30章 | 4.4 | ✅ | チャネル拡大（優先度7） |
| 戦略的改善ロードマップ（25章参照） | 25章 | 15.4 | ✅ | 本アーキテクチャへの影響サマリー |
...
| 2026-02-10 | 運用セクション: デプロイ/CI/lib同期/SQL検証エントリ追加 |
| 2026-02-10 | 技術基盤セクション: 3層防御/Codexレビューエントリ追加 |
| 2026-02-14 | セクション10「戦略的改善計画」追加（30章 3AI合同分析レポート連携） |
