...
# ★★★ v10.18.1: lib追加（summary生成、department_id対応）★★★
try:
    from lib import (
    ...
    USE_LIB = False
    print(f"⚠️ lib が見つかりません。フォールバック: {e}")
    ...
    "chatwork_task_create": {
        ...
        "params_schema": {
            ...
            "limit_date": {
                ...
                "required": False,
                "note": "「明日」→翌日、「明後日」→2日後、「来週金曜」→該当日に変換"
            },
            "limit_time": {
                "type": "time",
    ...
    try:
        ...
        if USE_LIB and body:
            try:
                ...
                print(f"📝 summary生成: {summary[:30] if summary else 'なし'}...")
            except Exception as e:
        ...
        with pool.connect() as conn:
            ...
            conn.commit()
        print(f"✅ タスクをDBに保存: task_id={task_data['task_id']}, summary={summary[:20] if summary else 'なし'}...")
        return True
    except Exception as e:
...
【判断の優先順位】
=======================================================
1. タスク関連のキーワード（追加/作成/依頼/お願い）があれば → chatwork_task_create
2. 人物情報を教えてくれていれば → save_memory
3. 人物について質問していれば → query_memory
4. 忘れてほしいと言われていれば → delete_memory
5. それ以外 → general_chat"""
