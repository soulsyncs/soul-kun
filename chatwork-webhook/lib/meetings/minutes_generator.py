...
# =============================================================================
# 講義スタイル議事録プロンプト（Vision 12.2.4準拠）
# =============================================================================
# ChatWork音声アップロード・Zoom VTT共通で使用。
# 設計書12.2.4の固定プロンプト要件を全て包含した改良版:
#   - 細かく具体的な要約 + 背景記載
#   - 例え話・実例・数字を省略しない
#   - 講義スタイル
#   - ■主題（00:00〜）セクション形式
...
def build_chatwork_minutes_prompt(
    ...
    """
    議事録プロンプトを構築する（ChatWork音声アップロード・Zoom VTT共通）。

    プレーンテキスト出力（講義スタイル＋時系列セクション）。
    Vision 12.2.4準拠のフォーマットで全ての議事録フローで使用。
