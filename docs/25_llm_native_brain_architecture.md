...
**バージョン:** v1.5.0
**作成日:** 2026-01-30
**最終更新:** 2026-01-31（モデル選定をGPT-5.2に更新、コスト見積もり改訂）
...
1. **何をするか**: 脳の中核にLLM（GPT-5.2 via OpenRouter）を常駐させ、意図理解・文脈補完をLLMの推論に委ねる
...
┃                                   ▼                                      ┃
┃  ┌──────────────────────────────────────────────────────────────────┐  ┃
┃  │                  ② LLM Brain（GPT-5.2 via OpenRouter）             │  ┃
┃  │                                                                    │  ┃
┃  │  【入力】                                                          │  ┃
...
ソウルくんの「思考」の中核。
GPT-5.2（OpenRouter経由）を使用して、ユーザーの意図を汲み取り、適切なToolを選択する。
※モデルは `lib/brain/llm_brain.py` の `DEFAULT_MODEL_OPENROUTER` で変更可能。
...
| 項目 | 月額見込み（5,000回会話） |
|------|-------------------------|
| GPT-5.2 API (OpenRouter) | 約9,000円（キャッシュあり） |
| Cloud Run | 約7,500円 |
| Supabase | 約5,500円 |
| Pinecone | 約1,500円 |
| その他 | 約500円 |
