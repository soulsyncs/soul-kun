...
| 項目 | 内容 |
|------|------|
| **この文書の役割** | Phase 2.5目標達成支援機能の詳細設計書 |
| **書くこと** | 目標達成支援の設計思想、ソウルシンクスのMVV、目標達成の技術（ナポレオン・ヒル/選択理論）、ソウルくんのキャラクター設定、goals/goal_progress/goal_reminders等のテーブル設計、目標設定対話フロー（WHY→WHAT→HOW）、通知設計、API設計 |
| **書かないこと** | ソウルくんの具体的なプロンプト集（→05_phase2-5_soulkun_prompts.md） |
| **SoT（この文書が正）** | 目標達成支援の3つの設計原則、目標設定対話フロー（3ステップ）、goals/goal_progress/goal_reminders/goal_setting_sessions等のDBスキーマ、17時問いかけ・18時リマインド・翌朝8時フィードバックの通知設計、機密区分ルール |
| **Owner** | Tech Lead |
| **更新トリガー** | 対話フローの変更、DBスキーマの変更、通知設計の変更、MVV・行動指針の更新 |

---

**v1.8 変更点（臨機応変な対応 - Adaptive Response Enhancement）:**
- 質問検出機能追加（？で終わる、「どうしたらいい」等のヘルプ要求）
- 困惑・迷い検出機能追加（「わからない」「難しい」等、全ステップ共通）
- 極端に短い回答の検出（<5文字、5-10文字の閾値）
- 具体性スコアリング強化（文字数、数値表現、期限表現、行動動詞）
- リトライ回数に応じたトーン変更（2回目: 優しい、3回目: 受け入れ）
- 新規テンプレート12種追加（help_question_*, help_confused_*, too_short等）
...
### 5.6 目標の削除・整理フロー（実運用必須）

**背景:**  
実運用では「重複」「未定」「相談中」「期限切れ」が必ず発生する。  
一覧の再掲だけでは整理が進まず、ユーザーが疲弊する。

**方針:**  
1) 一覧表示の後に「次の一手」を必ず提示する  
2) 番号指定 → 確認 → 実行の2ステップ  
3) 1回の会話で“整理が完了する体験”を作る
...
ソウルくん: 「了解ウル！削除する目標を番号で教えてほしいウル。
例: 1,3,5
同じ内容の重複がある場合は『重複を全部削除』でもOKウル。」

スタッフ: 「1,2,3」

ソウルくん: 「以下を削除するウル。間違いない？
1. 個人で100万円を受注する…
2. 個人で100万円を受注する…
3. 個人で100万円を受注する…
OKなら『削除する』と返信してウル。」
...
#### 5.6.4 期限切れ整理のルール（最低限）

- `period_end < 今日` の目標を「整理候補」として提示  
- ユーザー確認後に **status='cancelled'** に変更  
- 変更理由は **audit_logs** に記録

---

### 5.7 「次の一手」提示（伴走の要）

目標管理の価値は「記録」ではなく「前に進む行動」を生み出すこと。

**必須ルール:**  
進捗報告・相談・レビューのたびに「次にやるべきこと」を必ず1つ提示する。

#### 5.7.1 進捗報告後

```
「進捗は順調ウル。
次の一手: 今日は “見込み客2件に連絡” をやるウル？」
```

#### 5.7.2 相談後

```
「詰まりポイントは“行動が結果に繋がっていない”ウル。
