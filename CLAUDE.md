...
**次にやること:**
> 1. 本番ログ監視（1-3日）- 脳の判断ログを確認
> 2. フォールバック発動の有無を確認
> 3. 問題なければ旧コード削除（main.pyを約1000行削減予定）
...
| ~~★★☆~~ | ~~脳アーキテクチャ本番有効化~~ | ~~シャドウモード稼働中 → 段階的ロールアウト~~ | ~~1-2週間~~ | ✅ **完了 (v10.31.3)** |
| **★★☆** | **本番ログ監視・旧コード削除** | 脳の判断ログを確認、問題なければ旧コード削除 | 1-2週間 | - |
...
- **2026-01-27 10:30 JST**: 目標設定機能 DB制約修正 + 開始キーワード検出 (v10.31.6) ✅ **PR #213 マージ完了**
  ...
  - **バグ修正（DB制約違反）**:
    - `goal_setting_logs.result`: `cancelled` → `abandoned`（DB制約: accepted, retry, abandoned）
    - `goal_setting_sessions.status`: `cancelled` → `abandoned`（DB制約: in_progress, completed, abandoned）
    - 修正箇所: lib/goal_setting.py の4箇所
  - **機能追加（開始キーワード検出）**:
    - 「目標設定したい」「目標を立てたい」等のキーワードで新規セッション開始
    - 脳アーキテクチャより先にチェックし、目標設定フローにルーティング
    - 対応キーワード: 目標設定したい, 目標を設定したい, 目標を立てたい, 目標を決めたい, 目標設定, 目標登録, 今月の目標を設定, 個人目標を設定
  - **変更ファイル**:
    - `lib/goal_setting.py`: DB制約違反修正（4箇所）
    - `chatwork-webhook/lib/goal_setting.py`: 同期
    - `chatwork-webhook/main.py`: 開始キーワード検出追加（+20行）
  - **テスト**: Quality Checks全パス
  - **10の鉄則準拠**: DB制約を正しく守るよう修正

- **2026-01-27 10:15 JST**: Phase 2D設計書「会社の教えとして伝える」原則追加 ✅ **PR #212 マージ完了**
  - **実施者**: Claude Code
  - **概要**: CEO教え機能（Phase 2D）の設計書に「会社の教えとして伝える」原則を追加
  - **背景**: カズさんからの指示「スタッフにアドバイスするときに『俺が言っていた』って言っちゃダメで、『ソウルシンクスとして大事にすることは？』に言い換える設計にして」
  - **追加内容**:
    - **原則6追加**: 「会社の教えとして伝える」
      - スタッフへは「ソウルシンクスとして大事にしていること」と表現
      - 「カズさんが言っていた」とは絶対に言わない
      - 違反リスク: CEOの属人的カリスマに依存した組織になる
    - **表現ルール詳細**（セクション4.1.1新設）:
      - 禁止パターン: ❌「菊地さんはこう言っていたウル」「カズさんが〜」「社長が〜」
      - 正しいパターン: ✅「ソウルシンクスとして大事にしていることは〇〇ウル」
  - **変更ファイル**:
    - `docs/15_phase2d_ceo_learning.md`: 原則6追加、表現ルールセクション追加
  - **10の鉄則準拠**: 設計書のみ（DB操作なし）

- **2026-01-27 08:00 JST**: 脳アーキテクチャ本番バグ2件修正 (v10.31.5) ✅ **本番デプロイ完了**
  - **実施者**: Claude Code
  - **概要**: 本番監視で発見した脳アーキテクチャのバグ2件を修正
  - **バグ1: SQL構文エラー（state_manager.py）**
    - 症状: 状態遷移時に `syntax error at or near ":"` エラー
    - 原因: `:state_data::jsonb` という書き方がpg8000ドライバーで動かない
    - 修正: `CAST(:state_data AS jsonb)` に変更（3箇所）
  - **バグ2: dict属性エラー（understanding.py）**
    - 症状: `'dict' object has no attribute 'body'` エラー
    - 原因: `task.body` がオブジェクト前提だが、dictが渡されることがある
    - 修正: dict/オブジェクト両対応に変更（2箇所）
  - **デプロイ**: chatwork-webhook revision **00199-xoq**
  - **PR**: #210
  - **テスト**: 1,895件パス
  - **検証結果**: 起動ログ・実行ログともにエラーなし ✅
  - **10の鉄則準拠**: SQLインジェクション対策維持、フォールバック設計
...
- **2026-01-27 06:45 JST**: 脳アーキテクチャ 本番有効化 (v10.31.3) ✅ **本番デプロイ完了**
  - **実施者**: Claude Code
  - **概要**: 脳アーキテクチャ（BrainIntegration）をシャドウモードから本番モードに切り替え
  - **デプロイ内容**:
    | 項目 | 値 |
    |------|-----|
    | Cloud Function | chatwork-webhook |
    | リビジョン | 00195-zot |
    | モード | **enabled** (本番) |
  - **環境変数**:
    ```
    USE_BRAIN_ARCHITECTURE=true        ← shadow → true に変更
    BRAIN_SHADOW_LOGGING=false         ← true → false に変更
    BRAIN_FALLBACK_ENABLED=true        ← 維持（安全策）
    ```
  - **本番モードの動作**:
    - 全てのメッセージが**脳**を経由して処理される
    - 脳がエラーを起こした場合は**旧コードに自動フォールバック**
    - **5分以内にロールバック可能**（`USE_BRAIN_ARCHITECTURE=false`でデプロイ）
  - **事前確認**:
    - ユニットテスト: 1,895件全パス
    - 脳モジュール同期: 13ファイル全て同期確認
    - フォールバック機構: 4箇所で自動発動を確認
  - **検証結果**:
    - リビジョン健全: Ready, Active, ContainerHealthy
    - HTTP応答: 正常
    - テストリクエスト: 期待通りの処理
    - 起動時警告: あり（原因調査中、動作に影響なし）
  - **次のステップ**:
    1. 本番ログ監視（1-3日）: 脳の判断ログを確認
    2. フォールバック発動の有無を確認
    3. 問題なければ旧コード削除（main.pyを約1000行削減予定）
  - **ロールバック方法**: `USE_BRAIN_ARCHITECTURE=false`でデプロイ（約3分）
  - **10の鉄則準拠**: フォールバック設計、エラーハンドリング、ログ出力

- **2026-01-27 06:05 JST**: 脳アーキテクチャ シャドウモードデプロイ (v10.31.2) ✅ **本番デプロイ完了**
  - **実施者**: Claude Code
  - **概要**: 脳アーキテクチャ（BrainIntegration）をシャドウモードで本番有効化
  - **デプロイ内容**:
    | 項目 | 値 |
    |------|-----|
    | Cloud Function | chatwork-webhook |
  ...
  - **環境変数**:
    ...
    BRAIN_SHADOW_LOGGING=true
    BRAIN_FALLBACK_ENABLED=true
    ```
  - **シャドウモードの動作**:
    - 新しい脳アーキテクチャと従来フローが**並列実行**
    - ユーザーには**従来の結果**を返却
    - 両者の結果を**比較ログ**として記録（`[SHADOW]`タグ）
  - **ログ確認**: `✅ lib/brain loaded (v10.29.0), enabled=True, mode=shadow`
  - **テスト修正（PR #204）**:
    - `test_announcement_handler.py`: Phase A対応（organization_id → admin_config）
    - `test_brain_decision.py`: タイミングテストのジッター許容
    - 全テスト: 1895件パス
  - **次のステップ**:
    1. シャドウログ監視（1-3日）: Cloud Loggingで`[SHADOW]`ログを分析
    2. 段階的ロールアウト: 5% → 25% → 50%
    3. 全面有効化: `USE_BRAIN_ARCHITECTURE=true`
  - **ロールバック方法**: `USE_BRAIN_ARCHITECTURE=false`でデプロイ
  - **10の鉄則準拠**: フォールバック設計、エラーハンドリング、ログ出力
