...
| 項目 | 内容 |
|------|------|
| **この文書の役割** | Memory Framework（覚える能力）の詳細設計書 |
| **書くこと** | 記憶の種類、保存方法、検索方法、プライバシー考慮 |
| **書かないこと** | 脳アーキテクチャ全体（→25章）、DB実装詳細（→03章） |
| **SoT（この文書が正）** | Memory Frameworkの設計、記憶分類ルール |
| **Owner** | Tech Lead |
| **更新トリガー** | Memory機能の仕様変更時 |
