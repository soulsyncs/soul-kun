...
# =============================================================================
# Tool権限レベル定義（CLAUDE.md §8 権限レベル体系）
# =============================================================================
# Level 1: 業務委託（自部署のみ・制限あり）
# Level 2: 一般社員（自部署のみ）
# Level 3: リーダー/課長（自部署 + 直下部署）
# Level 4: 幹部/部長（全配下部署）
# Level 5: 管理部/取締役（全組織・最高機密除く）
# Level 6: 代表/CFO（全組織・全情報）
#
# 各ToolのSoT: このファイルが正。registry.pyのrequired_levelより優先される。
# 変更時はCLAUDE.md §8 権限レベル原則と整合させること。
TOOL_REQUIRED_LEVELS: Dict[str, int] = {
    # -------------------------------------------------------------------------
    # タスク管理（Level 1〜3）
    # -------------------------------------------------------------------------
    "chatwork_task_search": 1,     # 誰でも自分のタスクを確認できる
    "chatwork_task_complete": 1,   # 誰でも自分のタスクを完了できる
    "chatwork_task_create": 2,     # 一般社員以上：他者へのタスク作成
    "chatwork_task_edit": 2,       # 一般社員以上：タスク内容の変更
    "chatwork_task_delete": 3,     # リーダー以上：タスクの削除は影響大
    # -------------------------------------------------------------------------
    # ナレッジ・記憶（Level 1〜5）
    # -------------------------------------------------------------------------
    "query_knowledge": 1,          # 誰でも社内ナレッジを検索できる
    "list_knowledge": 2,           # 一般社員以上：ナレッジ一覧
    "query_memory": 2,             # 一般社員以上：記憶参照
    "save_memory": 2,              # 一般社員以上：記憶保存
    "learn_knowledge": 4,          # 幹部以上：ナレッジ追加は知識ベース変更
    "delete_memory": 4,            # 幹部以上：記憶削除は影響大
    "forget_knowledge": 5,         # 取締役以上：ナレッジ削除は不可逆
    # -------------------------------------------------------------------------
    # 目標管理（Level 1〜3）
    # -------------------------------------------------------------------------
    "goal_consult": 1,             # 誰でも目標を相談できる
    "goal_status_check": 1,        # 誰でも自分の目標状況を確認できる
    "goal_progress_report": 2,     # 一般社員以上：進捗報告
    "goal_registration": 2,        # 一般社員以上：目標登録
    "goal_review": 3,              # リーダー以上：部下の目標レビュー
    # -------------------------------------------------------------------------
    # 会話・情報取得（Level 1〜2）
    # -------------------------------------------------------------------------
    "general_conversation": 1,     # 誰でも会話できる
    "daily_reflection": 1,         # 誰でも日次振り返りできる
    "connection_query": 2,         # 一般社員以上：人脈・人物情報
    "query_org_chart": 2,          # 一般社員以上：組織図参照
    "web_search": 2,               # 一般社員以上：Web検索
    "calendar_read": 2,            # 一般社員以上：カレンダー参照
    "drive_search": 2,             # 一般社員以上：Driveファイル検索
    "past_meeting_query": 2,       # 一般社員以上：過去会議検索
    # -------------------------------------------------------------------------
    # 文書・コンテンツ生成（Level 2〜3）
    # -------------------------------------------------------------------------
    "generate_document": 2,        # 一般社員以上：文書生成
    "generate_image": 2,           # 一般社員以上：画像生成
    "file_create": 2,              # 一般社員以上：ファイル作成
    "meeting_transcription": 2,    # 一般社員以上：会議文字起こし
    "zoom_meeting_minutes": 2,     # 一般社員以上：Zoom議事録
    "schedule_management": 2,      # 一般社員以上：スケジュール管理
    "generate_video": 3,           # リーダー以上：動画生成（コスト大）
    # -------------------------------------------------------------------------
    # データ・レポート（Level 4〜5）：経営情報を扱う
    # -------------------------------------------------------------------------
    "data_search": 4,              # 幹部以上：データ検索（全社データへのアクセス）
    "data_aggregate": 4,           # 幹部以上：データ集計（経営指標）
    "report_generate": 4,          # 幹部以上：レポート生成（経営判断用）
    "csv_export": 4,               # 幹部以上：CSV出力（大量データ持ち出し）
    # -------------------------------------------------------------------------
    # 意思決定・一斉配信（Level 4〜5）
    # -------------------------------------------------------------------------
    "proposal_decision": 4,        # 幹部以上：提案・意思決定支援
    "announcement_create": 4,      # 幹部以上：全社お知らせ作成（一斉配信）
}

# Tool権限エラーメッセージ（権限不足時にソウルくんが返す）
TOOL_PERMISSION_DENIED_MESSAGE: str = (
    "ごめんウル🐺 その操作はLevel {required}以上の権限が必要ウル。"
    "権限がある方に依頼するか、上位の方に確認してほしいウル。"
)
