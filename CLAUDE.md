...
- **全入力・全出力は脳（Brain）を通る。バイパス禁止。例外なし。**
...
| Step | やること | 禁止事項 |
|------|---------|---------|
| 1. 計測 | 診断ログ（print/タイミング）**だけ**を入れてデプロイ | コードの修正をしてはいけない |
| 2. 観察 | 本番ログを読んで「どこが何秒かかっているか」の実データ取得 | 推測で原因を決めつけてはいけない |
| 3. 合議 | **同じログデータをCodexとGeminiにも見せて**、それぞれに原因分析と修正案を出させる。3者の修正案が一致したことを確認してから次へ進む | 自分一人の判断だけで修正に着手してはいけない |
| 4. 報告 | カズさんに「ログ証拠 + 3者一致の修正案」を見せる | 「〜だと思う」「〜が怪しい」で報告してはいけない |
| 5. 修正 | 証拠と合議に基づいて、原因箇所**だけ**を修正 | 1回のデプロイで複数の推測修正を混ぜてはいけない |
| 6. 検証 | デプロイ後にログで改善を確認 | 「直したはず」で終わらせてはいけない |

**重要: Codex/Geminiのコードレビューは「推測の精度が高い推測」にすぎない。計測データの代わりにはならない。ただし、計測データに基づく修正案の検証には必ずCodex/Geminiを使うこと。**

> 制定理由: 2026-02-11のPR #462〜#466で、推測ベースの修正を4回繰り返して4回失敗。計測して初めて真の原因（context_builder.build()のDBタイムアウト未設定）が特定できた。
