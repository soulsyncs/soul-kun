> ⚠️ **DEPRECATED - 参照禁止**
>
> このファイルは **`docs/25_llm_native_brain_architecture.md`** に統合されました。
>
> | 統合先 | 25章 第9章「リスク対策」+ 付録17.6「将来の拡張」 |
> |--------|---------------------------------------------|
> | 理由 | 学習基盤設計の統合 |
> | 日付 | 2026-01-30 |
>
> **👉 参照すべきファイル:** [25章 LLM常駐型脳アーキテクチャ](../25_llm_native_brain_architecture.md)

---

# Phase 2E: 学習基盤（Learning Foundation）

## 設計書情報

| 項目 | 内容 |
|------|------|
| バージョン | 1.1.0 |
| 作成日 | 2026-01-27 |
| 更新日 | 2026-01-27 |
| 作成者 | Claude Code |
| ステータス | ダブルチェック完了 |
| 前提Phase | Phase 2D（CEO Learning）完了 |
| 次Phase | Phase 2F（結果からの学習） |

---

## 1. 概要

### 1.1 このPhaseの目的

**「カズさんに教えてもらったら、ちゃんと覚えて次から直す」**

現在のソウルくんは、間違いを指摘されても「すみませんウル」と謝るだけで、
同じ間違いを繰り返す。これは「学習」ではなく「反応」に過ぎない。

Phase 2Eでは、明示的なフィードバック（「違うよ」「こうだよ」）を
受け取り、記憶し、次回から適用する仕組みを構築する。

### 1.2 解決する問題

| 現状の問題 | Phase 2E後 |
|-----------|-----------|
| 「違うよ」と言われても次も同じ間違い | 一度教わったら二度と間違えない |
| 何を覚えたか分からない | 覚えたことを一覧で確認できる |
| 教え方が分からない | 自然な会話で教えられる |
| 誰が教えたか分からない | 誰がいつ教えたか記録される |
| CEO教えと矛盾する可能性 | **CEO教えが常に優先される** |

### 1.3 対応する能力項目

| # | 能力 | 現状 | 目標 |
|---|------|------|------|
| 4-1 | 明示的学習 | ❌ | ○ |
| 4-2 | フィードバック学習 | ❌ | ○ |
| 4-7 | 忘却防止 | ❌ | ○ |
| 4-8 | 知識の更新 | ❌ | ○ |

### 1.4 設計原則

1. **自然な教え方**: 特別なコマンドなしで、会話の中で教えられる
2. **即座の反映**: 教えた直後から適用される
3. **永続的な記憶**: 一度教えたら忘れない（明示的に削除しない限り）
4. **透明性**: 何を覚えているか、いつでも確認できる
5. **安全性**: 間違った学習は修正・削除できる
6. **CEO教え優先**: Phase 2Dで蓄積したCEO教えと矛盾する学習は適用されない
7. **将来拡張性**: Phase 2F-2Oへの接続ポイントを備える

---

## 2. 学習の種類と範囲

### 2.1 学習カテゴリ

Phase 2Eで対応する学習カテゴリを定義する。

| カテゴリ | 説明 | 例 |
|---------|------|-----|
| `alias` | 別名・略称 | 「麻美」=「渡部麻美」 |
| `preference` | 好み・やり方 | 「報告は箇条書きで」 |
| `fact` | 事実・情報 | 「Aプロジェクトの担当は田中」 |
| `rule` | ルール・決まり | 「金曜は定例会議がある」 |
| `correction` | 間違いの修正 | 「タスクの期限は翌営業日」 |
| `context` | 文脈・背景 | 「今月は繁忙期」 |
| `relationship` | 人間関係 | 「佐藤と鈴木は同期」 |
| `procedure` | 手順・やり方 | 「請求書は経理に回す」 |

### 2.2 学習のスコープ

学習した内容の適用範囲を定義する。

| スコープ | 説明 | 例 |
|---------|------|-----|
| `global` | 全員に適用 | 会社のルール、共通知識 |
| `user` | 特定ユーザーのみ | 個人の好み、略称 |
| `room` | 特定ルームのみ | プロジェクト固有の情報 |
| `temporary` | 期間限定 | 「今月は〇〇」 |

### 2.3 学習の権限レベル（Phase 2D連携）

学習の教示者による優先度を定義する。

| 権限レベル | 説明 | 優先度 | 上書き可能な対象 |
|-----------|------|--------|----------------|
| `ceo` | CEOからの教え（Phase 2D経由） | 最高 | 全て |
| `manager` | 管理職からの教え | 高 | user, system |
| `user` | 一般ユーザーからの教え | 中 | system |
| `system` | システム自動学習（Phase 2F） | 低 | なし |

**重要**: `ceo`レベルの学習はPhase 2Dの`ceo_teachings`テーブルから派生し、
他の学習よりも常に優先される。

### 2.4 学習の優先度解決

同じ状況で複数の学習が該当する場合の優先順位。

```
優先度（高い順）:
1. CEO教え（ceo_teachings）← Phase 2Dから参照
2. 権限レベルが高い学習（authority_level）
3. ユーザー固有の学習（user scope）
4. ルーム固有の学習（room scope）
5. 最新の学習（newer created_at）
6. グローバルな学習（global scope）
```

---

## 3. フィードバック検出

### 3.1 フィードバックパターン

ユーザーの発言から「これは教えている」ことを検出するパターン。

#### 3.1.1 直接的な修正パターン

```
パターン1: 「〇〇は△△だよ」
例: 「麻美は渡部麻美のことだよ」
抽出: trigger=「麻美」, correction=「渡部麻美」, category=alias

パターン2: 「〇〇じゃなくて△△」
例: 「週報じゃなくて日報だよ」
抽出: wrong=「週報」, correct=「日報」, category=correction

パターン3: 「〇〇のことは△△って呼んで」
例: 「渡部麻美のことは麻美って呼んで」
抽出: target=「渡部麻美」, alias=「麻美」, category=alias

パターン4: 「〇〇=△△」「〇〇イコール△△」
例: 「SS=ソウルシンクス」
抽出: alias=「SS」, full=「ソウルシンクス」, category=alias
```

#### 3.1.2 ルール教示パターン

```
パターン5: 「〇〇の時は△△して」
例: 「急ぎの時は先にDMで連絡して」
抽出: condition=「急ぎの時」, action=「先にDMで連絡」, category=rule

パターン6: 「〇〇は△△がルール」
例: 「報告は毎日17時がルール」
抽出: subject=「報告」, rule=「毎日17時」, category=rule

パターン7: 「〇〇しないで」「〇〇は禁止」
例: 「勝手にタスク削除しないで」
抽出: prohibited_action=「勝手にタスク削除」, category=rule
```

#### 3.1.3 好み教示パターン

```
パターン8: 「〇〇が好き」「〇〇の方がいい」
例: 「箇条書きの方がいい」
抽出: preference=「箇条書き」, category=preference

パターン9: 「〇〇でお願い」
例: 「敬語でお願い」
抽出: preference=「敬語」, category=preference

パターン10: 「いつも〇〇で」
例: 「いつも要点3つでまとめて」
抽出: preference=「要点3つでまとめる」, category=preference
```

#### 3.1.4 事実教示パターン

```
パターン11: 「〇〇は△△」（事実の陳述）
例: 「Aプロジェクトの担当は田中さん」
抽出: subject=「Aプロジェクトの担当」, fact=「田中さん」, category=fact

パターン12: 「覚えておいて、〇〇」
例: 「覚えておいて、来月から新オフィス」
抽出: fact=「来月から新オフィス」, category=fact

パターン13: 「〇〇だから覚えておいて」
例: 「山田さんは育休中だから覚えておいて」
抽出: fact=「山田さんは育休中」, category=fact
```

#### 3.1.5 文脈依存パターン

```
パターン14: 直前の間違いへの指摘
例:
  ソウルくん: 「田中さんにタスクを作成したウル」
  ユーザー: 「田中じゃなくて佐藤」
抽出: previous_action=「田中さんにタスク」, correction=「佐藤」

パターン15: 暗黙の修正
例:
  ソウルくん: 「明日の会議は10時ですウル」
  ユーザー: 「11時だよ」
抽出: wrong=「10時」, correct=「11時」, context=「明日の会議」
```

### 3.2 フィードバック検出フロー

```
┌─────────────────────────────────────────────────────────────┐
│                   フィードバック検出フロー                    │
├─────────────────────────────────────────────────────────────┤
│                                                              │
│   ユーザーメッセージ受信                                      │
│         │                                                    │
│         ▼                                                    │
│   ┌───────────────────────┐                                 │
│   │ 1. パターンマッチング   │                                 │
│   │    15種のパターンで検査 │                                 │
│   └───────────┬───────────┘                                 │
│               │                                              │
│         マッチあり?                                           │
│         ├─ No ──→ 通常処理へ                                │
│         │                                                    │
│         ▼ Yes                                                │
│   ┌───────────────────────┐                                 │
│   │ 2. 文脈分析            │                                 │
│   │    直前の会話を確認    │                                 │
│   └───────────┬───────────┘                                 │
│               │                                              │
│         ▼                                                    │
│   ┌───────────────────────┐                                 │
│   │ 3. 学習内容の抽出      │                                 │
│   │    カテゴリ、スコープ  │                                 │
│   │    トリガー、修正内容  │                                 │
│   └───────────┬───────────┘                                 │
│               │                                              │
│         ▼                                                    │
│   ┌───────────────────────┐                                 │
│   │ 4. CEO教えとの整合性   │ ← ★ Phase 2D連携               │
│   │    矛盾がないか確認    │                                 │
│   └───────────┬───────────┘                                 │
│               │                                              │
│         矛盾あり?                                             │
│         ├─ Yes ──→ CEO教え優先を通知                        │
│         │          「CEOの方針と異なるため覚えられないウル」 │
│         │                                                    │
│         ▼ No                                                 │
│   ┌───────────────────────┐                                 │
│   │ 5. 確信度の算出        │                                 │
│   │    パターン明確さ      │                                 │
│   │    文脈の一致度        │                                 │
│   └───────────┬───────────┘                                 │
│               │                                              │
│         確信度 ≥ 0.7?                                        │
│         ├─ No ──→ 確認モード                                │
│         │         「〇〇と覚えていいですか？」               │
│         │                                                    │
│         ▼ Yes                                                │
│   ┌───────────────────────┐                                 │
│   │ 6. 既存学習との矛盾検出 │ ← ★ 矛盾検出                   │
│   │    同じtriggerの学習を │                                 │
│   │    確認し、矛盾を検出  │                                 │
│   └───────────┬───────────┘                                 │
│               │                                              │
│         矛盾あり?                                             │
│         ├─ Yes ──→ 旧学習を無効化（superseded_by設定）      │
│         │                                                    │
│         ▼ No                                                 │
│   ┌───────────────────────┐                                 │
│   │ 7. 学習の保存          │                                 │
│   │    DBに記録            │                                 │
│   └───────────┬───────────┘                                 │
│               │                                              │
│         ▼                                                    │
│   ┌───────────────────────┐                                 │
│   │ 8. 確認メッセージ      │                                 │
│   │    「覚えたウル！」    │                                 │
│   └───────────────────────┘                                 │
│                                                              │
└─────────────────────────────────────────────────────────────┘
```

### 3.3 確認モード

確信度が低い場合、ユーザーに確認を求める。

```
確認メッセージ例:

【別名の場合】
「『麻美』と言ったら『渡部麻美さん』のことだと覚えていいウル？」

【ルールの場合】
「『急ぎの時は先にDMで連絡する』というルールを覚えていいウル？」

【好みの場合】
「カズさんは『箇条書き』がお好みと覚えていいウル？」

ユーザーの回答:
- 「うん」「そう」「はい」「OK」→ 学習を保存
- 「違う」「いや」「ちがう」→ 学習をキャンセル、詳細を質問
```

### 3.4 CEO教え矛盾時の対応

Phase 2DのCEO教えと矛盾する学習を検出した場合：

```
【CEO教えと矛盾する場合のメッセージ】
「ごめんウル！ソウルシンクスとして大事にしていることと異なるため、
 この内容は覚えることができないウル🐺

 ソウルシンクスのルール:
 「報告は毎日17時に行う」

 カズさんに確認が必要な場合は、管理部チャットでお知らせできるウル」
```

---

## 4. 学習の保存

### 4.1 データモデル

#### 4.1.1 brain_learnings テーブル

```sql
CREATE TABLE brain_learnings (
    -- 識別情報
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    organization_id UUID NOT NULL,

    -- 学習内容
    category VARCHAR(50) NOT NULL,
    -- 'alias', 'preference', 'fact', 'rule', 'correction',
    -- 'context', 'relationship', 'procedure'

    -- トリガー（いつ適用するか）
    trigger_type VARCHAR(50) NOT NULL,
    -- 'keyword', 'pattern', 'context', 'always'
    trigger_value TEXT NOT NULL,
    -- キーワード、正規表現パターン、文脈条件

    -- 学習内容の詳細
    learned_content JSONB NOT NULL,
    learned_content_version INTEGER NOT NULL DEFAULT 1,
    -- {
    --   "type": "alias",
    --   "from": "麻美",
    --   "to": "渡部麻美",
    --   "description": "麻美は渡部麻美の略称"
    -- }

    -- スコープ
    scope VARCHAR(50) NOT NULL DEFAULT 'global',
    -- 'global', 'user', 'room', 'temporary'
    scope_target_id VARCHAR(100),
    -- user scope の場合: account_id
    -- room scope の場合: room_id

    -- ★ 権限レベル（Phase 2D連携）
    authority_level VARCHAR(20) NOT NULL DEFAULT 'user',
    -- 'ceo', 'manager', 'user', 'system'
    related_ceo_teaching_id UUID,
    -- Phase 2D ceo_teachings との関連

    -- 有効期間（temporary scopeの場合）
    valid_from TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
    valid_until TIMESTAMP WITH TIME ZONE,

    -- 教えた人
    taught_by_account_id VARCHAR(50) NOT NULL,
    taught_by_name VARCHAR(100),
    taught_in_room_id VARCHAR(50),

    -- 元のメッセージ
    source_message TEXT,
    source_context JSONB,
    -- 直前の会話など

    -- 検出情報
    detection_pattern VARCHAR(100),
    detection_confidence DECIMAL(3,2),

    -- 適用状況
    is_active BOOLEAN DEFAULT true,
    applied_count INTEGER DEFAULT 0,
    last_applied_at TIMESTAMP WITH TIME ZONE,
    success_count INTEGER DEFAULT 0,
    failure_count INTEGER DEFAULT 0,

    -- 上書き情報
    supersedes_id UUID REFERENCES brain_learnings(id),
    superseded_by_id UUID REFERENCES brain_learnings(id),

    -- ★ Phase 2G準備: 知識グラフ
    related_learning_ids UUID[],
    relationship_type VARCHAR(50),
    -- 'implies', 'contradicts', 'extends', 'specializes'
    confidence_decay_rate DECIMAL(5,4) DEFAULT 0.0001,
    -- 時間経過による確信度低下率

    -- ★ Phase 2H準備: 自己認識
    unknown_pattern TEXT,
    -- 「分からなかった」パターンを記録
    clarification_count INTEGER DEFAULT 0,
    -- 確認を求めた回数

    -- ★ Phase 2N準備: 自己最適化
    effectiveness_score DECIMAL(3,2)
        CHECK (effectiveness_score IS NULL OR
               (effectiveness_score >= 0.00 AND effectiveness_score <= 1.00)),
    last_effectiveness_check TIMESTAMP WITH TIME ZONE,
    decision_impact VARCHAR(20),
    -- 'positive', 'negative', 'neutral'

    -- 監査
    classification VARCHAR(20) DEFAULT 'internal',
    created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP
);

-- インデックス
CREATE INDEX idx_learnings_org ON brain_learnings(organization_id);
CREATE INDEX idx_learnings_category ON brain_learnings(organization_id, category);
CREATE INDEX idx_learnings_trigger ON brain_learnings(trigger_type, trigger_value);
CREATE INDEX idx_learnings_scope ON brain_learnings(organization_id, scope, scope_target_id);
CREATE INDEX idx_learnings_active ON brain_learnings(organization_id, is_active)
    WHERE is_active = true;
CREATE INDEX idx_learnings_taught_by ON brain_learnings(taught_by_account_id);
CREATE INDEX idx_learnings_updated ON brain_learnings(updated_at DESC);
CREATE INDEX idx_learnings_authority ON brain_learnings(authority_level);
CREATE INDEX idx_learnings_ceo_teaching ON brain_learnings(related_ceo_teaching_id)
    WHERE related_ceo_teaching_id IS NOT NULL;

-- Phase 2F準備: 暗黙フィードバック検出用
CREATE INDEX idx_learnings_implicit ON brain_learnings(detection_pattern)
    WHERE detection_pattern LIKE 'implicit_%';

-- Phase 2N準備: 効果測定用
CREATE INDEX idx_learnings_effectiveness ON brain_learnings(effectiveness_score DESC)
    WHERE effectiveness_score IS NOT NULL;

-- 矛盾検出用（高速化）
CREATE INDEX idx_learnings_trigger_active ON brain_learnings(trigger_value, is_active)
    WHERE is_active = true;

-- トリガー（全文検索用）
CREATE INDEX idx_learnings_content ON brain_learnings
    USING GIN(learned_content jsonb_path_ops);

-- CHECK制約
ALTER TABLE brain_learnings ADD CONSTRAINT chk_learnings_category
    CHECK (category IN ('alias', 'preference', 'fact', 'rule',
                        'correction', 'context', 'relationship', 'procedure'));

ALTER TABLE brain_learnings ADD CONSTRAINT chk_learnings_scope
    CHECK (scope IN ('global', 'user', 'room', 'temporary'));

ALTER TABLE brain_learnings ADD CONSTRAINT chk_learnings_trigger_type
    CHECK (trigger_type IN ('keyword', 'pattern', 'context', 'always'));

ALTER TABLE brain_learnings ADD CONSTRAINT chk_learnings_authority_level
    CHECK (authority_level IN ('ceo', 'manager', 'user', 'system'));

ALTER TABLE brain_learnings ADD CONSTRAINT chk_learnings_relationship_type
    CHECK (relationship_type IS NULL OR relationship_type IN
           ('implies', 'contradicts', 'extends', 'specializes'));

ALTER TABLE brain_learnings ADD CONSTRAINT chk_learnings_decision_impact
    CHECK (decision_impact IS NULL OR decision_impact IN
           ('positive', 'negative', 'neutral'));

COMMENT ON TABLE brain_learnings IS 'ソウルくんの学習内容（Phase 2E）';
```

#### 4.1.2 brain_learning_logs テーブル

学習の適用ログを記録する。

```sql
CREATE TABLE brain_learning_logs (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    organization_id UUID NOT NULL,
    learning_id UUID NOT NULL REFERENCES brain_learnings(id),
    learning_version INTEGER NOT NULL DEFAULT 1,

    -- 適用状況
    applied_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
    applied_in_room_id VARCHAR(50),
    applied_for_account_id VARCHAR(50),

    -- トリガー情報
    trigger_message TEXT,
    trigger_context JSONB,
    context_hash VARCHAR(64),
    -- 文脈の一意識別子（重複検出用）

    -- 結果
    was_successful BOOLEAN,
    result_description TEXT,
    response_latency_ms INTEGER,
    -- 応答時間（性能分析用）

    -- フィードバック（もしあれば）
    feedback_received BOOLEAN DEFAULT false,
    feedback_positive BOOLEAN,
    feedback_message TEXT,
    user_feedback_at TIMESTAMP WITH TIME ZONE,

    -- 監査
    created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP
);

CREATE INDEX idx_learning_logs_learning ON brain_learning_logs(learning_id);
CREATE INDEX idx_learning_logs_org ON brain_learning_logs(organization_id);
CREATE INDEX idx_learning_logs_applied ON brain_learning_logs(applied_at DESC);
CREATE INDEX idx_learning_logs_feedback ON brain_learning_logs(learning_id, feedback_received)
    WHERE feedback_received = true;
CREATE INDEX idx_learning_logs_context_hash ON brain_learning_logs(context_hash);

COMMENT ON TABLE brain_learning_logs IS '学習適用ログ（Phase 2E）';
```

### 4.2 learned_content の構造

カテゴリごとのJSONB構造を定義。

#### 4.2.1 alias（別名）

```json
{
  "type": "alias",
  "from": "麻美",
  "to": "渡部麻美",
  "bidirectional": false,
  "description": "麻美は渡部麻美の略称"
}
```

#### 4.2.2 preference（好み）

```json
{
  "type": "preference",
  "subject": "報告形式",
  "preference": "箇条書き",
  "priority": "high",
  "description": "報告は箇条書きを好む"
}
```

#### 4.2.3 fact（事実）

```json
{
  "type": "fact",
  "subject": "Aプロジェクトの担当",
  "value": "田中さん",
  "confidence": "high",
  "source": "カズさんからの直接教示",
  "description": "Aプロジェクトの担当は田中さん"
}
```

#### 4.2.4 rule（ルール）

```json
{
  "type": "rule",
  "condition": "急ぎの時",
  "action": "先にDMで連絡する",
  "priority": "high",
  "exceptions": [],
  "description": "急ぎの時は先にDMで連絡するルール"
}
```

#### 4.2.5 correction（修正）

```json
{
  "type": "correction",
  "wrong_pattern": "タスクの期限を当日に設定",
  "correct_pattern": "タスクの期限を翌営業日に設定",
  "reason": "当日は厳しすぎる",
  "description": "タスク期限は当日ではなく翌営業日"
}
```

#### 4.2.6 context（文脈）

```json
{
  "type": "context",
  "subject": "今月",
  "context": "繁忙期",
  "implications": ["残業が多い", "急ぎの依頼が増える"],
  "valid_until": "2026-01-31",
  "description": "今月は繁忙期"
}
```

#### 4.2.7 relationship（関係）

```json
{
  "type": "relationship",
  "person1": "佐藤",
  "person2": "鈴木",
  "relationship": "同期",
  "notes": "仲が良い",
  "description": "佐藤と鈴木は同期"
}
```

#### 4.2.8 procedure（手順）

```json
{
  "type": "procedure",
  "task": "請求書処理",
  "steps": [
    "1. 内容確認",
    "2. 経理に回す",
    "3. 承認を待つ"
  ],
  "notes": "急ぎの場合は直接経理に声をかける",
  "description": "請求書は経理に回す手順"
}
```

---

## 5. 学習の適用

### 5.1 適用フロー

```
┌─────────────────────────────────────────────────────────────┐
│                     学習適用フロー                           │
├─────────────────────────────────────────────────────────────┤
│                                                              │
│   メッセージ処理開始                                          │
│         │                                                    │
│         ▼                                                    │
│   ┌───────────────────────┐                                 │
│   │ 0. CEO教えの参照       │ ← ★ Phase 2D連携              │
│   │    ceo_teachings参照   │                                 │
│   └───────────┬───────────┘                                 │
│               │                                              │
│         ▼                                                    │
│   ┌───────────────────────┐                                 │
│   │ 1. 関連学習の検索      │                                 │
│   │    - キーワードマッチ  │                                 │
│   │    - パターンマッチ    │                                 │
│   │    - 文脈マッチ        │                                 │
│   └───────────┬───────────┘                                 │
│               │                                              │
│         学習あり?                                             │
│         ├─ No ──→ 通常処理へ                                │
│         │                                                    │
│         ▼ Yes                                                │
│   ┌───────────────────────┐                                 │
│   │ 2. CEO教えとの整合性   │ ← ★ 矛盾する学習を除外        │
│   │    矛盾する学習を除外  │                                 │
│   └───────────┬───────────┘                                 │
│               │                                              │
│         ▼                                                    │
│   ┌───────────────────────┐                                 │
│   │ 3. スコープ確認        │                                 │
│   │    - user/room/global │                                 │
│   │    - 有効期間          │                                 │
│   └───────────┬───────────┘                                 │
│               │                                              │
│         適用可能?                                             │
│         ├─ No ──→ 通常処理へ                                │
│         │                                                    │
│         ▼ Yes                                                │
│   ┌───────────────────────┐                                 │
│   │ 4. 優先度解決          │                                 │
│   │    複数学習がある場合  │                                 │
│   │    authority_level順  │                                 │
│   └───────────┬───────────┘                                 │
│               │                                              │
│         ▼                                                    │
│   ┌───────────────────────┐                                 │
│   │ 5. 学習の適用          │                                 │
│   │    カテゴリ別の処理    │                                 │
│   └───────────┬───────────┘                                 │
│               │                                              │
│         ▼                                                    │
│   ┌───────────────────────┐                                 │
│   │ 6. 適用ログの記録      │                                 │
│   │    - learning_id      │                                 │
│   │    - 適用結果          │                                 │
│   └───────────────────────┘                                 │
│                                                              │
└─────────────────────────────────────────────────────────────┘
```

### 5.2 カテゴリ別の適用方法

#### 5.2.1 alias（別名）の適用

```python
# メッセージ内の「麻美」を見つけたら
if "麻美" in message:
    # 学習を検索
    learning = find_learning(trigger_value="麻美", category="alias")
    if learning:
        # 内部的に「渡部麻美」として処理
        resolved_name = learning.learned_content["to"]  # "渡部麻美"
        # 以降の処理では resolved_name を使用
```

#### 5.2.2 preference（好み）の適用

```python
# 報告を生成する時
def generate_report(content, user_id):
    # ユーザーの好みを検索
    preference = find_learning(
        category="preference",
        scope="user",
        scope_target_id=user_id
    )
    if preference and preference.learned_content["subject"] == "報告形式":
        format_style = preference.learned_content["preference"]
        # 好みの形式で報告を生成
        return format_report(content, style=format_style)
```

#### 5.2.3 rule（ルール）の適用

```python
# アクションを実行する前に
def check_rules(action, context):
    # 関連ルールを検索
    rules = find_learnings(category="rule")
    for rule in rules:
        condition = rule.learned_content["condition"]
        if matches_condition(context, condition):
            required_action = rule.learned_content["action"]
            # ルールに従った行動を追加
            apply_rule(required_action)
```

#### 5.2.4 correction（修正）の適用

```python
# アクションを実行する前に
def apply_corrections(intended_action):
    # 関連する修正を検索
    corrections = find_learnings(category="correction")
    for correction in corrections:
        wrong_pattern = correction.learned_content["wrong_pattern"]
        if matches_pattern(intended_action, wrong_pattern):
            correct_pattern = correction.learned_content["correct_pattern"]
            # 修正後のアクションに変更
            return apply_correction(intended_action, correct_pattern)
    return intended_action
```

### 5.3 脳アーキテクチャとの統合

学習層は、脳の処理フローの各段階で参照される。

```
┌─────────────────────────────────────────────────────────────┐
│                    SoulkunBrain                              │
├─────────────────────────────────────────────────────────────┤
│                                                              │
│   0. CEO教え参照（Phase 2D）                                 │
│      └── ceo_teachings を常に最優先で参照                    │
│                                                              │
│   1. 理解層（Understanding）                                  │
│      └── alias学習を適用（名前の解決）                        │
│      └── context学習を参照（背景理解）                        │
│                                                              │
│   2. 判断層（Decision）                                       │
│      └── rule学習を適用（ルールに従う）                       │
│      └── correction学習を適用（間違いを避ける）               │
│      └── ★ CEO教えと矛盾しないことを確認                     │
│                                                              │
│   3. 実行層（Execution）                                      │
│      └── procedure学習を適用（正しい手順）                    │
│      └── preference学習を適用（好みに合わせる）               │
│                                                              │
│   4. 学習層（Learning）                                       │
│      └── 新しい学習の検出・保存                               │
│      └── 適用結果のログ記録                                   │
│      └── ★ CEO教え矛盾時は学習を拒否                         │
│                                                              │
└─────────────────────────────────────────────────────────────┘
```

---

## 6. 確認メッセージ

### 6.1 学習時の確認

学習が検出された時の確認メッセージ。

```
【別名を学習した場合】
「覚えたウル！『麻美』と言ったら『渡部麻美さん』のことですねウル🐺」

【ルールを学習した場合】
「覚えたウル！『急ぎの時は先にDMで連絡する』ルールですねウル🐺」

【好みを学習した場合】
「覚えたウル！カズさんは『箇条書き』がお好みですねウル🐺」

【事実を学習した場合】
「覚えたウル！『Aプロジェクトの担当は田中さん』ですねウル🐺」

【修正を学習した場合】
「覚えたウル！『タスク期限は当日じゃなくて翌営業日』ですねウル🐺
 次からそうするウル！」

【確信度が低い場合】
「『麻美』は『渡部麻美さん』のことだと覚えていいウル？」

【CEO教えと矛盾する場合】
「ごめんウル！ソウルシンクスとして大事にしていることと異なるため、
 この内容は覚えることができないウル🐺」
```

### 6.2 適用時の言及

学習を適用した時、必要に応じて言及する。

```
【初回適用時】
「渡部麻美さん（カズさんが『麻美』と呼んでいる方）のタスクですねウル」

【ルール適用時】
「急ぎとのことなので、先にDMでも連絡したウル🐺」

【好み適用時】
（好みに合わせた形式で出力するが、特に言及しない）

【修正適用時】
「タスクの期限は翌営業日にしたウル🐺」
```

---

## 7. 学習の管理

### 7.1 学習一覧の確認

ユーザーが「何を覚えてる？」と聞いた時の応答。

```
【コマンド例】
「ソウルくん、何覚えてる？」
「覚えてること教えて」
「学習内容を見せて」

【応答例】
「ソウルくんが覚えていることウル🐺

📝 別名（3件）
・麻美 → 渡部麻美さん
・SS → ソウルシンクス
・タカ → 高橋さん

📋 ルール（2件）
・急ぎの時は先にDMで連絡する
・金曜は定例会議がある

💡 好み（カズさん）（1件）
・報告は箇条書きで

📌 事実（2件）
・Aプロジェクトの担当は田中さん
・今月は繁忙期

全部で8件覚えているウル！
詳しく見たい項目があれば教えてウル」
```

### 7.2 学習の削除

間違った学習を削除する。

```
【コマンド例】
「麻美の別名、忘れて」
「急ぎDMのルール、削除して」
「覚えてる〇〇を忘れて」

【応答例】
「了解ウル！『麻美 → 渡部麻美さん』を忘れたウル🐺」

【確認が必要な場合】
「どの学習を忘れればいいウル？
1. 麻美 → 渡部麻美さん（別名）
2. 麻美は経理担当（事実）
番号で教えてウル」
```

### 7.3 学習の修正

既存の学習を修正する。

```
【コマンド例】
「麻美は渡部麻美じゃなくて、浅井麻美」
「さっき教えたの違った、〇〇だよ」

【処理】
1. 既存の学習を superseded_by で無効化
2. 新しい学習を作成（supersedes で旧学習を参照）

【応答例】
「修正したウル！『麻美』は『浅井麻美さん』のことですねウル🐺
 前に覚えた『渡部麻美さん』は忘れたウル」
```

---

## 8. コード設計

### 8.1 クラス構成

```
lib/brain/
├── learning/
│   ├── __init__.py           # エクスポート
│   ├── detector.py           # フィードバック検出
│   ├── extractor.py          # 学習内容の抽出
│   ├── repository.py         # DB操作
│   ├── applier.py            # 学習の適用
│   ├── manager.py            # 学習の管理（一覧、削除、修正）
│   ├── patterns.py           # 検出パターン定義
│   ├── conflict_detector.py  # ★ 矛盾検出（新規）
│   ├── authority_resolver.py # ★ 権限レベル解決（新規）
│   └── effectiveness_tracker.py  # ★ 効果追跡（Phase 2N準備）
```

### 8.2 主要クラス

#### 8.2.1 FeedbackDetector

```python
class FeedbackDetector:
    """フィードバックを検出するクラス"""

    def __init__(self, patterns: List[DetectionPattern]):
        self.patterns = patterns

    def detect(
        self,
        message: str,
        context: Optional[ConversationContext] = None
    ) -> Optional[FeedbackDetectionResult]:
        """
        メッセージからフィードバックを検出する

        Returns:
            FeedbackDetectionResult or None
        """
        for pattern in self.patterns:
            match = pattern.match(message, context)
            if match:
                return FeedbackDetectionResult(
                    pattern=pattern,
                    match=match,
                    confidence=self._calculate_confidence(match, context)
                )
        return None

    def _calculate_confidence(
        self,
        match: PatternMatch,
        context: Optional[ConversationContext]
    ) -> float:
        """確信度を計算"""
        base_confidence = match.pattern.base_confidence

        # 文脈が一致すれば確信度アップ
        if context and self._context_supports(match, context):
            base_confidence += 0.1

        # 直前に間違いがあれば確信度アップ
        if context and context.has_recent_error:
            base_confidence += 0.1

        return min(base_confidence, 1.0)
```

#### 8.2.2 LearningExtractor

```python
class LearningExtractor:
    """学習内容を抽出するクラス"""

    def extract(
        self,
        detection_result: FeedbackDetectionResult,
        message: str,
        context: Optional[ConversationContext] = None,
        taught_by_authority: str = 'user'
    ) -> Learning:
        """
        検出結果から学習内容を抽出する
        """
        category = self._determine_category(detection_result)

        return Learning(
            category=category,
            trigger_type=self._determine_trigger_type(detection_result),
            trigger_value=self._extract_trigger(detection_result, message),
            learned_content=self._extract_content(detection_result, message, category),
            scope=self._determine_scope(detection_result, context),
            authority_level=taught_by_authority,  # ★ 権限レベル
            detection_pattern=detection_result.pattern.name,
            detection_confidence=detection_result.confidence,
            source_message=message,
            source_context=context.to_dict() if context else None
        )
```

#### 8.2.3 LearningRepository

```python
class LearningRepository:
    """学習のDB操作"""

    def __init__(self, pool, organization_id: str):
        self.pool = pool
        self.organization_id = organization_id

    async def save(self, learning: Learning, taught_by: str) -> str:
        """学習を保存"""
        query = """
            INSERT INTO brain_learnings (
                organization_id, category, trigger_type, trigger_value,
                learned_content, learned_content_version, scope, scope_target_id,
                authority_level, related_ceo_teaching_id,
                taught_by_account_id, source_message, source_context,
                detection_pattern, detection_confidence
            ) VALUES (
                :org_id, :category, :trigger_type, :trigger_value,
                :learned_content, :learned_content_version, :scope, :scope_target_id,
                :authority_level, :related_ceo_teaching_id,
                :taught_by, :source_message, :source_context,
                :detection_pattern, :detection_confidence
            ) RETURNING id
        """
        # ... 実装

    async def find_applicable(
        self,
        message: str,
        user_id: str,
        room_id: str
    ) -> List[Learning]:
        """適用可能な学習を検索（authority_level順）"""
        query = """
            SELECT * FROM brain_learnings
            WHERE organization_id = :org_id
              AND is_active = true
              AND (valid_until IS NULL OR valid_until > CURRENT_TIMESTAMP)
              AND (
                  scope = 'global'
                  OR (scope = 'user' AND scope_target_id = :user_id)
                  OR (scope = 'room' AND scope_target_id = :room_id)
              )
            ORDER BY
                CASE authority_level
                    WHEN 'ceo' THEN 1
                    WHEN 'manager' THEN 2
                    WHEN 'user' THEN 3
                    ELSE 4
                END,
                CASE scope
                    WHEN 'user' THEN 1
                    WHEN 'room' THEN 2
                    ELSE 3
                END,
                created_at DESC
        """
        # ... 実装

    async def deactivate(self, learning_id: str, superseded_by: str = None):
        """学習を無効化"""
        # ... 実装
```

#### 8.2.4 LearningApplier

```python
class LearningApplier:
    """学習を適用するクラス"""

    def __init__(
        self,
        repository: LearningRepository,
        ceo_teaching_repository: CEOTeachingRepository  # ★ Phase 2D連携
    ):
        self.repository = repository
        self.ceo_teaching_repository = ceo_teaching_repository

    async def apply_to_understanding(
        self,
        message: str,
        user_id: str,
        room_id: str
    ) -> Tuple[str, List[AppliedLearning]]:
        """
        理解層での学習適用
        - alias: 名前の解決
        - context: 背景情報の追加
        """
        # ★ まずCEO教えを取得
        ceo_teachings = await self.ceo_teaching_repository.get_relevant(
            message, self.repository.organization_id
        )

        # 学習を取得
        learnings = await self.repository.find_applicable(
            message, user_id, room_id
        )

        # ★ CEO教えと矛盾する学習を除外
        learnings = self._filter_conflicting_with_ceo(learnings, ceo_teachings)

        applied = []
        resolved_message = message

        for learning in learnings:
            if learning.category == 'alias':
                resolved_message = self._apply_alias(
                    resolved_message, learning
                )
                applied.append(AppliedLearning(learning, 'alias_resolved'))

        return resolved_message, applied

    def _filter_conflicting_with_ceo(
        self,
        learnings: List[Learning],
        ceo_teachings: List[CEOTeaching]
    ) -> List[Learning]:
        """CEO教えと矛盾する学習を除外"""
        if not ceo_teachings:
            return learnings

        filtered = []
        for learning in learnings:
            is_conflicting = False
            for teaching in ceo_teachings:
                if self._conflicts_with(learning, teaching):
                    is_conflicting = True
                    break
            if not is_conflicting:
                filtered.append(learning)
        return filtered

    async def apply_to_decision(
        self,
        decision: Decision,
        context: BrainContext
    ) -> Tuple[Decision, List[AppliedLearning]]:
        """
        判断層での学習適用
        - rule: ルールの適用
        - correction: 修正の適用
        """
        # ... 実装
```

#### 8.2.5 LearningManager

```python
class LearningManager:
    """学習の管理（一覧、削除、修正）"""

    def __init__(self, repository: LearningRepository):
        self.repository = repository

    async def list_learnings(
        self,
        category: Optional[str] = None,
        scope: Optional[str] = None
    ) -> List[Learning]:
        """学習一覧を取得"""
        # ... 実装

    async def delete_learning(self, learning_id: str) -> bool:
        """学習を削除"""
        # ... 実装

    async def update_learning(
        self,
        learning_id: str,
        new_content: dict
    ) -> Learning:
        """学習を更新（旧学習を無効化し新学習を作成）"""
        # ... 実装

    async def format_list_for_display(
        self,
        learnings: List[Learning]
    ) -> str:
        """表示用にフォーマット"""
        # ... 実装
```

#### 8.2.6 ConflictDetector（新規）

```python
class ConflictDetector:
    """学習間の矛盾を検出するクラス"""

    def __init__(self, repository: LearningRepository):
        self.repository = repository

    async def detect_conflicts(
        self,
        new_learning: Learning,
        existing_learnings: List[Learning] = None
    ) -> List[ConflictInfo]:
        """
        新しい学習が既存の学習と矛盾するかを検出

        矛盾パターン:
        1. 同じtrigger_valueで異なるlearned_content
        2. categoryがcorrectionで、同じwrong_patternに異なるcorrect_pattern
        3. ruleカテゴリで、同じconditionに異なるaction
        """
        if existing_learnings is None:
            existing_learnings = await self.repository.find_by_trigger(
                new_learning.trigger_value
            )

        conflicts = []
        for existing in existing_learnings:
            if self._is_conflicting(new_learning, existing):
                conflicts.append(ConflictInfo(
                    new_learning=new_learning,
                    existing_learning=existing,
                    conflict_type=self._determine_conflict_type(new_learning, existing),
                    resolution_suggestion=self._suggest_resolution(new_learning, existing)
                ))
        return conflicts

    async def resolve_conflict(
        self,
        conflict: ConflictInfo,
        resolution_strategy: str = 'newer_wins'
    ) -> Resolution:
        """
        矛盾を解決

        戦略:
        - newer_wins: 新しい学習を優先（旧学習をsupersede）
        - higher_authority: authority_levelが高い方を優先
        - confirm_user: ユーザーに確認
        """
        if resolution_strategy == 'newer_wins':
            # 旧学習を無効化
            await self.repository.deactivate(
                conflict.existing_learning.id,
                superseded_by=conflict.new_learning.id
            )
            return Resolution(
                action='superseded',
                kept_learning=conflict.new_learning,
                removed_learning=conflict.existing_learning
            )
        elif resolution_strategy == 'higher_authority':
            if conflict.new_learning.authority_level_priority > \
               conflict.existing_learning.authority_level_priority:
                # 新学習が高権限なので旧を無効化
                await self.repository.deactivate(
                    conflict.existing_learning.id,
                    superseded_by=conflict.new_learning.id
                )
                return Resolution(action='superseded', ...)
            else:
                # 旧学習が高権限なので新学習を拒否
                return Resolution(action='rejected', ...)
        else:
            return Resolution(action='needs_confirmation', ...)

    def _is_conflicting(self, new: Learning, existing: Learning) -> bool:
        """2つの学習が矛盾するかを判定"""
        # 同じカテゴリで同じトリガーだが内容が異なる
        if new.category != existing.category:
            return False
        if new.trigger_value != existing.trigger_value:
            return False
        return new.learned_content != existing.learned_content
```

#### 8.2.7 AuthorityResolver（新規）

```python
class AuthorityResolver:
    """権限レベル解決クラス"""

    AUTHORITY_PRIORITY = {
        'ceo': 1,
        'manager': 2,
        'user': 3,
        'system': 4
    }

    def get_priority(self, authority_level: str) -> int:
        """権限レベルの優先度を取得（数字が小さいほど高優先）"""
        return self.AUTHORITY_PRIORITY.get(authority_level, 5)

    def can_override(self, new_authority: str, existing_authority: str) -> bool:
        """新しい権限が既存の権限を上書きできるかを判定"""
        return self.get_priority(new_authority) <= self.get_priority(existing_authority)

    def determine_authority_from_account(
        self,
        account_id: str,
        organization_id: str,
        pool
    ) -> str:
        """アカウントIDから権限レベルを決定"""
        # CEOのアカウントIDかどうかを確認
        # 管理者テーブルを参照して判定
        # ... 実装
```

#### 8.2.8 EffectivenessTracker（Phase 2N準備）

```python
class EffectivenessTracker:
    """学習効果追跡クラス（Phase 2N準備）"""

    def __init__(self, repository: LearningRepository):
        self.repository = repository

    async def record_application_result(
        self,
        learning_id: str,
        was_successful: bool,
        feedback_positive: Optional[bool] = None
    ):
        """学習適用結果を記録"""
        # brain_learning_logsに記録
        # success_count / failure_count を更新
        pass

    async def calculate_effectiveness(
        self,
        learning_id: str
    ) -> float:
        """学習の効果スコアを計算（0.0-1.0）"""
        # 成功率、フィードバック、最近の使用状況から計算
        # effectiveness_score を更新
        pass

    async def get_underperforming_learnings(
        self,
        threshold: float = 0.5
    ) -> List[Learning]:
        """効果の低い学習を取得"""
        pass

    async def suggest_improvements(
        self,
        learning: Learning
    ) -> List[ImprovementSuggestion]:
        """学習の改善提案を生成（Phase 2N用）"""
        pass
```

### 8.3 core.py への統合

```python
# lib/brain/core.py

class SoulkunBrain:
    def __init__(self, ...):
        # 既存の初期化...

        # Phase 2E: 学習基盤
        self.feedback_detector = FeedbackDetector(DETECTION_PATTERNS)
        self.learning_extractor = LearningExtractor()
        self.learning_repository = LearningRepository(pool, org_id)
        self.conflict_detector = ConflictDetector(self.learning_repository)
        self.authority_resolver = AuthorityResolver()
        self.learning_applier = LearningApplier(
            self.learning_repository,
            self.ceo_teaching_repository  # Phase 2D
        )
        self.learning_manager = LearningManager(self.learning_repository)
        self.effectiveness_tracker = EffectivenessTracker(self.learning_repository)

    async def process_message(self, message, room_id, account_id, sender_name):
        # 1. フィードバック検出
        feedback = self.feedback_detector.detect(
            message,
            self._get_conversation_context(room_id)
        )

        if feedback:
            # フィードバックが検出された場合
            return await self._handle_feedback(
                feedback, message, room_id, account_id, sender_name
            )

        # 2. 通常処理（学習を適用しながら）
        # 2.1 理解層で学習適用
        resolved_message, applied_in_understanding = \
            await self.learning_applier.apply_to_understanding(
                message, account_id, room_id
            )

        # 2.2 理解
        understanding = await self._understand(resolved_message, context)

        # 2.3 判断層で学習適用
        decision, applied_in_decision = \
            await self.learning_applier.apply_to_decision(
                self._decide(understanding, context),
                context
            )

        # 2.4 実行
        result = await self._execute(decision)

        # 2.5 適用ログ記録 + 効果追跡
        await self._log_applied_learnings(
            applied_in_understanding + applied_in_decision
        )

        return result

    async def _handle_feedback(self, feedback, message, room_id, account_id, sender_name):
        """フィードバックを処理"""
        # 権限レベルを決定
        authority_level = self.authority_resolver.determine_authority_from_account(
            account_id, self.org_id, self.pool
        )

        # 学習内容を抽出
        learning = self.learning_extractor.extract(
            feedback, message,
            self._get_conversation_context(room_id),
            taught_by_authority=authority_level
        )

        # ★ CEO教えとの整合性チェック
        ceo_teachings = await self.ceo_teaching_repository.get_relevant(
            message, self.org_id
        )
        ceo_conflict = self._check_ceo_conflict(learning, ceo_teachings)
        if ceo_conflict:
            return BrainResponse(
                message=self._format_ceo_conflict_message(ceo_conflict),
                action_taken="learning_rejected_ceo_conflict"
            )

        # ★ 既存学習との矛盾チェック
        conflicts = await self.conflict_detector.detect_conflicts(learning)
        if conflicts:
            # 矛盾がある場合は自動解決（newer_wins）
            for conflict in conflicts:
                await self.conflict_detector.resolve_conflict(
                    conflict,
                    resolution_strategy='newer_wins'
                )

        if feedback.confidence >= 0.7:
            # 確信度が高い：直接学習
            await self.learning_repository.save(learning, account_id)

            return BrainResponse(
                message=self._format_learning_confirmation(learning),
                action_taken="learning_saved"
            )
        else:
            # 確信度が低い：確認モード
            return BrainResponse(
                message=self._format_learning_question(feedback),
                action_taken="learning_confirmation_requested",
                requires_response=True
            )
```

---

## 9. テスト計画

### 9.1 ユニットテスト

```python
# tests/test_learning_detector.py

class TestFeedbackDetector:
    """フィードバック検出のテスト"""

    def test_detect_alias_pattern_1(self):
        """「〇〇は△△だよ」パターン"""
        detector = FeedbackDetector(DETECTION_PATTERNS)
        result = detector.detect("麻美は渡部麻美のことだよ")

        assert result is not None
        assert result.pattern.name == "alias_definition"
        assert result.confidence >= 0.7

    def test_detect_alias_pattern_2(self):
        """「〇〇じゃなくて△△」パターン"""
        detector = FeedbackDetector(DETECTION_PATTERNS)
        result = detector.detect("田中じゃなくて佐藤だよ")

        assert result is not None
        assert result.pattern.name == "correction"

    def test_detect_rule_pattern(self):
        """ルール教示パターン"""
        detector = FeedbackDetector(DETECTION_PATTERNS)
        result = detector.detect("急ぎの時は先にDMで連絡して")

        assert result is not None
        assert result.pattern.name == "rule_definition"

    def test_no_feedback_in_normal_message(self):
        """通常メッセージでは検出しない"""
        detector = FeedbackDetector(DETECTION_PATTERNS)
        result = detector.detect("今日の会議は何時から？")

        assert result is None
```

```python
# tests/test_learning_extractor.py

class TestLearningExtractor:
    """学習内容抽出のテスト"""

    def test_extract_alias(self):
        """別名の抽出"""
        extractor = LearningExtractor()
        detection = FeedbackDetectionResult(
            pattern=PATTERNS["alias_definition"],
            match={"from": "麻美", "to": "渡部麻美"},
            confidence=0.9
        )

        learning = extractor.extract(detection, "麻美は渡部麻美のことだよ")

        assert learning.category == "alias"
        assert learning.learned_content["from"] == "麻美"
        assert learning.learned_content["to"] == "渡部麻美"

    def test_extract_with_authority_level(self):
        """権限レベル付き抽出"""
        extractor = LearningExtractor()
        learning = extractor.extract(
            detection, message,
            taught_by_authority='manager'
        )
        assert learning.authority_level == 'manager'
```

```python
# tests/test_learning_applier.py

class TestLearningApplier:
    """学習適用のテスト"""

    async def test_apply_alias_in_understanding(self):
        """理解層での別名適用"""
        # 事前に学習を保存
        await repository.save(Learning(
            category="alias",
            trigger_type="keyword",
            trigger_value="麻美",
            learned_content={"from": "麻美", "to": "渡部麻美"}
        ))

        applier = LearningApplier(repository, ceo_repo)
        resolved, applied = await applier.apply_to_understanding(
            "麻美のタスク教えて",
            user_id="1234",
            room_id="5678"
        )

        assert "渡部麻美" in resolved or len(applied) > 0

    async def test_ceo_teaching_takes_precedence(self):
        """CEO教えが学習より優先される"""
        # CEO教え: 報告は毎日17時
        await ceo_repo.save(CEOTeaching(
            statement="報告は毎日17時に行う",
            category="rule"
        ))
        # 一般学習: 報告は毎週月曜
        await repository.save(Learning(
            category="rule",
            learned_content={"condition": "報告", "action": "毎週月曜"},
            authority_level='user'
        ))

        applier = LearningApplier(repository, ceo_repo)
        # CEO教えと矛盾する学習は除外される
        learnings = await applier._filter_conflicting_with_ceo(...)
        assert len(learnings) == 0
```

```python
# tests/test_conflict_detector.py

class TestConflictDetector:
    """矛盾検出のテスト"""

    async def test_detect_alias_conflict(self):
        """同じ略称に異なる正式名称"""
        existing = Learning(
            trigger_value="麻美",
            learned_content={"from": "麻美", "to": "渡部麻美"}
        )
        new = Learning(
            trigger_value="麻美",
            learned_content={"from": "麻美", "to": "浅井麻美"}
        )

        detector = ConflictDetector(repository)
        conflicts = await detector.detect_conflicts(new, [existing])

        assert len(conflicts) == 1
        assert conflicts[0].conflict_type == "content_mismatch"

    async def test_resolve_conflict_newer_wins(self):
        """newer_wins戦略で解決"""
        detector = ConflictDetector(repository)
        resolution = await detector.resolve_conflict(
            conflict,
            resolution_strategy='newer_wins'
        )

        assert resolution.action == 'superseded'
        # 旧学習が無効化されていることを確認

    async def test_no_conflict_different_trigger(self):
        """異なるトリガーは矛盾しない"""
        existing = Learning(trigger_value="麻美", ...)
        new = Learning(trigger_value="崇樹", ...)

        conflicts = await detector.detect_conflicts(new, [existing])
        assert len(conflicts) == 0
```

```python
# tests/test_authority_resolver.py

class TestAuthorityResolver:
    """権限レベル解決のテスト"""

    def test_ceo_has_highest_priority(self):
        """CEOが最高優先度"""
        resolver = AuthorityResolver()
        assert resolver.get_priority('ceo') < resolver.get_priority('manager')
        assert resolver.get_priority('manager') < resolver.get_priority('user')
        assert resolver.get_priority('user') < resolver.get_priority('system')

    def test_can_override_lower_authority(self):
        """低い権限は上書き可能"""
        resolver = AuthorityResolver()
        assert resolver.can_override('ceo', 'user') == True
        assert resolver.can_override('user', 'ceo') == False
```

### 9.2 統合テスト

```python
# tests/test_learning_integration.py

class TestLearningIntegration:
    """学習機能の統合テスト"""

    async def test_full_learning_cycle(self):
        """学習→適用の完全サイクル"""
        brain = SoulkunBrain(pool=pool, org_id="test")

        # 1. 教える
        response1 = await brain.process_message(
            message="麻美は渡部麻美のことだよ",
            room_id="123",
            account_id="456",
            sender_name="カズ"
        )
        assert "覚えた" in response1.message

        # 2. 適用を確認
        response2 = await brain.process_message(
            message="麻美のタスク教えて",
            room_id="123",
            account_id="456",
            sender_name="カズ"
        )
        # 「渡部麻美」として処理されることを確認
        assert "渡部麻美" in response2.message or response2.action_taken == "task_search"

    async def test_learning_persistence(self):
        """学習の永続性"""
        brain1 = SoulkunBrain(pool=pool, org_id="test")

        # 教える
        await brain1.process_message(
            message="タカは高橋さんのことだよ",
            room_id="123",
            account_id="456",
            sender_name="カズ"
        )

        # 別のbrainインスタンスで確認
        brain2 = SoulkunBrain(pool=pool, org_id="test")

        response = await brain2.process_message(
            message="タカのタスク教えて",
            room_id="123",
            account_id="456",
            sender_name="カズ"
        )

        # 新しいインスタンスでも学習が適用される
        assert "高橋" in response.message or response.action_taken == "task_search"

    async def test_ceo_teaching_conflict_rejected(self):
        """CEO教えと矛盾する学習は拒否される"""
        brain = SoulkunBrain(pool=pool, org_id="test")

        # CEO教えを設定
        await ceo_repo.save(CEOTeaching(
            statement="報告は毎日17時に行う"
        ))

        # 矛盾する学習を試みる
        response = await brain.process_message(
            message="報告は毎週月曜でいいよ",
            room_id="123",
            account_id="789",  # 一般ユーザー
            sender_name="山田"
        )

        assert "覚えることができない" in response.message
        assert response.action_taken == "learning_rejected_ceo_conflict"

    async def test_supersedes_chain(self):
        """学習の上書きチェーン"""
        brain = SoulkunBrain(pool=pool, org_id="test")

        # 学習1
        await brain.process_message("麻美は渡部麻美だよ", ...)
        # 学習2（学習1を上書き）
        await brain.process_message("麻美は浅井麻美だよ", ...)
        # 学習3（学習2を上書き）
        await brain.process_message("麻美は山田麻美だよ", ...)

        # 最新の学習のみがアクティブ
        learnings = await repository.find_by_trigger("麻美")
        active = [l for l in learnings if l.is_active]
        assert len(active) == 1
        assert active[0].learned_content["to"] == "山田麻美"
```

```python
# tests/test_learning_performance.py

class TestLearningPerformance:
    """パフォーマンステスト"""

    async def test_search_with_10000_learnings(self):
        """10,000件の学習から検索"""
        # 10,000件のダミー学習を作成
        for i in range(10000):
            await repository.save(Learning(
                trigger_value=f"keyword_{i}",
                learned_content={"value": f"content_{i}"}
            ))

        start = time.time()
        await applier.apply_to_understanding(
            "keyword_5000のタスク",
            user_id="test",
            room_id="test"
        )
        elapsed = (time.time() - start) * 1000

        assert elapsed < 100  # 100ms以内

    async def test_concurrent_learning_writes(self):
        """同時に10件の学習を書き込み"""
        tasks = [
            repository.save(Learning(trigger_value=f"concurrent_{i}", ...))
            for i in range(10)
        ]

        # デッドロックなく完了することを確認
        results = await asyncio.gather(*tasks, return_exceptions=True)
        errors = [r for r in results if isinstance(r, Exception)]
        assert len(errors) == 0
```

### 9.3 テストケース一覧

| カテゴリ | テストケース数 | 内容 |
|---------|--------------|------|
| 検出 | 30 | 15パターン × 正常/異常 |
| 抽出 | 16 | 8カテゴリ × 正常/異常 |
| 保存 | 10 | CRUD操作 |
| 適用 | 20 | 各層での適用 |
| 管理 | 10 | 一覧/削除/修正 |
| 矛盾検出 | 15 | 矛盾検出・解決 ★追加 |
| Phase 2D統合 | 10 | CEO教え優先 ★追加 |
| 権限レベル | 5 | 権限解決 ★追加 |
| supersedes連鎖 | 5 | 上書き連鎖 ★追加 |
| パフォーマンス | 5 | 大量データ、並行処理 ★追加 |
| 統合 | 10 | E2Eフロー |
| **合計** | **136** | |

---

## 10. マイグレーション計画

### 10.1 DBマイグレーション

```sql
-- migrations/phase2e_learning_foundation.sql

BEGIN;

-- 1. brain_learnings テーブル作成
-- （4.1.1 の定義を使用）

-- 2. brain_learning_logs テーブル作成
-- （4.1.2 の定義を使用）

-- 3. インデックス作成
-- （4.1.1, 4.1.2 のインデックス定義を使用）

-- 4. トリガー作成
CREATE TRIGGER update_brain_learnings_updated_at
    BEFORE UPDATE ON brain_learnings
    FOR EACH ROW
    EXECUTE FUNCTION update_updated_at_column();

COMMIT;
```

### 10.2 コードデプロイ

```
1. lib/brain/learning/ ディレクトリを作成
2. 各モジュールを実装
   - detector.py
   - extractor.py
   - repository.py
   - applier.py
   - manager.py
   - patterns.py
   - conflict_detector.py ★新規
   - authority_resolver.py ★新規
   - effectiveness_tracker.py ★新規
3. lib/brain/core.py を更新
4. chatwork-webhook/lib/brain/ に同期
5. テスト実行（136件パス確認）
6. PR作成 → Quality Checks → マージ
7. DBマイグレーション実行
8. chatwork-webhook デプロイ
```

### 10.3 Feature Flag

```python
# 環境変数
USE_LEARNING_FOUNDATION = os.environ.get("USE_LEARNING_FOUNDATION", "false")

# core.py での分岐
if USE_LEARNING_FOUNDATION == "true":
    # Phase 2E の学習機能を使用
    feedback = self.feedback_detector.detect(message, context)
    if feedback:
        return await self._handle_feedback(feedback, ...)
else:
    # 従来の処理（学習機能なし）
    pass
```

---

## 11. 成功基準

### 11.1 機能的成功基準

| # | 基準 | 計測方法 |
|---|------|---------|
| 1 | 15種のパターンで学習を検出できる | ユニットテスト |
| 2 | 8カテゴリの学習を正しく保存できる | ユニットテスト |
| 3 | 保存した学習が次回から適用される | 統合テスト |
| 4 | 学習一覧を確認できる | 手動テスト |
| 5 | 学習を削除・修正できる | 手動テスト |
| 6 | CEO教えと矛盾する学習は拒否される | 統合テスト ★追加 |
| 7 | 既存学習との矛盾は自動解決される | 統合テスト ★追加 |

### 11.2 品質基準

| # | 基準 | 目標値 |
|---|------|--------|
| 1 | テストカバレッジ | 80%以上 |
| 2 | 全テストパス | 136件 |
| 3 | 検出精度（適合率） | 90%以上 |
| 4 | 検出精度（再現率） | 80%以上 |

### 11.3 運用基準

| # | 基準 | 目標値 |
|---|------|--------|
| 1 | 学習処理の応答時間 | 2秒以内 |
| 2 | 学習適用の応答時間追加 | 100ms以内 |
| 3 | DB容量（1ヶ月あたり） | 10MB以下 |
| 4 | 10,000件学習時の検索時間 | 100ms以内 ★追加 |

---

## 12. リスクと対策

### 12.1 技術的リスク

| リスク | 影響 | 対策 |
|--------|------|------|
| 誤検出（本来の会話を学習と誤認） | 不要な学習が保存される | 確信度閾値0.7、確認モード |
| 見逃し（学習を検出できない） | 教えても覚えない | パターンの継続的追加 |
| 矛盾する学習 | 古い学習と新しい学習が衝突 | supersedes/superseded_byで管理 |
| 性能劣化 | 学習が増えると遅くなる | インデックス最適化、キャッシュ |
| CEO教えとの矛盾 | 会社方針と異なる学習 | CEO教え優先チェック ★追加 |

### 12.2 運用リスク

| リスク | 影響 | 対策 |
|--------|------|------|
| 間違った学習 | 誤動作 | 削除機能、確認メッセージ |
| 学習の乱用 | 意図しない動作 | CEO/管理者のみ学習可能（オプション） |
| 個人情報の学習 | プライバシー問題 | classification=confidential |

---

## 13. 今後の拡張（Phase 2F以降との連携）

### 13.1 Phase 2F（結果からの学習）との連携

```
Phase 2E: 明示的フィードバック「違うよ」→ 学習
Phase 2F: 暗黙のフィードバック（削除された、無視された）→ 学習

両者は同じ brain_learnings テーブルを使用し、
detection_pattern で区別する:
- Phase 2E: "explicit_*"
- Phase 2F: "implicit_*"
```

### 13.2 Phase 2G（記憶の強化）との連携

```
Phase 2E: 個別の学習
Phase 2G: 学習同士の関連付け（知識グラフ）

準備済みフィールド:
- related_learning_ids: 関連する学習のID配列
- relationship_type: implies, contradicts, extends, specializes
- confidence_decay_rate: 時間経過による確信度低下

例:
- 「麻美」→「渡部麻美」（Phase 2E）
- 「渡部麻美」→「経理担当」（Phase 2E）
- 「麻美」→「経理担当」を自動推論（Phase 2G）
```

### 13.3 Phase 2H（自己認識）との連携

```
Phase 2E: 明示的に教えられた知識
Phase 2H: 「分からなかった」パターンの認識

準備済みフィールド:
- unknown_pattern: 「分からなかった」パターンを記録
- clarification_count: 確認を求めた回数

例:
- 確信度が低かった質問パターンを記録
- 何度も確認を求めているパターンを分析
- 「〇〇については詳しくないウル」と自己認識
```

### 13.4 Phase 2N（自己最適化）との連携

```
Phase 2E: 学習の保存・適用
Phase 2N: 学習効果の測定・最適化

準備済みフィールド:
- effectiveness_score: 学習の効果スコア (0.0-1.0)
- last_effectiveness_check: 最終効果測定日時
- decision_impact: positive, negative, neutral

EffectivenessTrackerクラスで:
- 学習適用結果を記録
- 効果スコアを計算
- 効果の低い学習を特定
- 改善提案を生成
```

---

## 14. 変更履歴

| バージョン | 日付 | 変更内容 |
|-----------|------|---------|
| 1.0.0 | 2026-01-27 | 初版作成 |
| 1.1.0 | 2026-01-27 | ダブルチェック反映: Phase 2D連携、矛盾検出、権限レベル、Phase 2F-2O接続ポイント追加 |

---

## 15. 承認

| 役割 | 名前 | 日付 | 承認 |
|------|------|------|------|
| 設計者 | Claude Code | 2026-01-27 | ✅ |
| ダブルチェック | Claude Code | 2026-01-27 | ✅ |
| レビュアー | - | - | 📋 |
| 承認者（カズさん） | - | - | 📋 |
