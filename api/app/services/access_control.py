...
class AccessControlService:
    ...
    async def compute_accessible_departments(
        ...
        try:
            ...
            logger.error(f"Failed to compute accessible departments for user {user_id}: {e}")
            return []  # 空のリストを返す（安全側に倒す）
    ...
    async def get_user_role_level(self, user_id: str) -> int:
        ...
        try:
            ...
            logger.error(f"Failed to get role level for user {user_id}: {e}")
            return 2  # デフォルト値（一般社員）を返す
    ...
    async def get_all_departments(self, organization_id: str) -> List[str]:
        ...
        try:
            ...
            logger.error(f"Failed to get all departments for organization {organization_id}: {e}")
            return []  # 空のリストを返す
    ...
    async def get_all_descendants(self, dept_id: str, organization_id: str) -> List[str]:
        ...
        try:
            ...
            logger.error(f"Failed to get descendants for department {dept_id}: {e}")
            return []  # 空のリストを返す（安全側に倒す）
    ...
    async def get_direct_children(self, dept_id: str, organization_id: str) -> List[str]:
        ...
        try:
            ...
            logger.error(f"Failed to get direct children for department {dept_id}: {e}")
            return []  # 空のリストを返す
    ...
    async def can_access_department(
        ...
        try:
            ...
            logger.error(f"Failed to check department access for user {user_id}, department {department_id}: {e}")
            return False  # アクセス拒否（安全側に倒す）
    ...
    async def can_access_task(
        ...
        try:
            ...
            logger.error(f"Failed to check task access for user {user_id}, task department {task_department_id}: {e}")
            return False  # アクセス拒否（安全側に倒す）
...
def get_user_role_level_sync(conn, user_id: str) -> int:
    ...
    try:
        ...
        logger.error(f"Failed to get role level for user {user_id} (sync): {e}")
        return 2  # デフォルト値（一般社員）を返す
...
def compute_accessible_departments_sync(
    ...
    try:
        ...
        logger.error(f"Failed to compute accessible departments for user {user_id} (sync): {e}")
        return []  # 空のリストを返す（安全側に倒す）
