...
- **2026-01-26 07:15 JST**: タスク要約切り詰め改善 (v10.25.6) ✅ **PR #135/PR #137 マージ完了**
  - **実施者**: Claude Code
  - **問題**: タスク要約がURL途中で切れたり、動詞形・接続表現で中途半端に終わる
  - **修正内容**:
    - **URL除去**: 要約表示には不要かつ途中で切れると見栄えが悪いためURLを事前除去
    - **URL除去後整形**: 「に をお願い」等の不自然なパターンを「をお願い」に整形
    - **途切れ末尾パターン拡充**: 動詞形（してい、している、しており、されて等）を検出・削除
    - **接続表現対応**: 「について」「として」「により」「において」「に対して」「に関して」を有効パターンとして保持
    - **アクション動詞拡充**: 18語→26語（連携、準備、整理、調査、相談、依頼、追加、更新）
    - **省略記号対応**: 最終手段として切り詰め時に「...」を付与
    - **ヘルパー関数追加**: `_remove_incomplete_ending()`で末尾不完全パターンを再帰的に削除
  - **変更ファイル**:
    - `lib/text_utils.py`: `prepare_task_display_text()`大幅改善
    - `tests/test_text_utils_lib.py`: 9件の新規テスト追加
    - 6つのCloud Functionsにlib/text_utils.py同期
  - **テスト**: 1,085件全パス（うちv10.25.6新規9件）
  - **10の鉄則準拠**: DB操作なし（該当なし）
