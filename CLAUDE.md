...
```
soul-kun/
├── main.py                  # 既存Flask/Cloud Functions（2,615行）
├── lib/                     # 【新規】共通ライブラリ
...
あなた（Claude Code）は、git commit する前に、必ず以下を実行してください。
...
git commit を実行する前に、必ずカズさんに以下を確認してください：
...
内容に問題なければ、git commit します。」

カズさんが「OK」または「いいよ」と言ったら、git commit してください。
カズさんが「ちょっと待って」と言ったら、修正してから再度確認してください。
