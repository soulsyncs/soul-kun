...
【グラフ構造】
    ...
    state_check
        │
        ├── has_active_session → route_session → END
        │
        └── no_session → build_context → llm_inference → guardian_check
                                                              │
                                                ┌─────────────┼──────────────┐
                                                │             │              │
                                           handle_block  handle_confirm  text_only
                                                │             │              │
                                               END           END         text_response → END
                                                                            │
                                                                       execute_tool
                                                                            │
                                                               ┌────────────┴────────────┐
