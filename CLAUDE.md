...
**変更作業の前に必ず [`docs/00_Goals_and_Principles.md`](docs/00_Goals_and_Principles.md) を読むこと。**

このドキュメントは「単一の信頼できるソース（Single Source of Truth）」であり、以下の8項目が定義されています:
...
**制約**:
- 目的/DoD/制約/権限/ログ方針と整合しない変更は禁止
- 不確実なら止まって相談（推測で進めない）
- 破壊的変更（DB/API/env/ログ形式）は原則禁止
