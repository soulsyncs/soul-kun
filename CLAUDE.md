...
### 3-2. コーディング時の17項目チェックリスト【v10.60追加】

**コードを書くとき・レビューするとき、必ずこの17項目を意識すること。**
Codexレビュー（3パス）でも同じ17項目をチェックする。予防（ここ）と検出（Codex）の二重防御。
...
```
[Layer 1: 予防] コード作成時に17項目チェックリスト（§3-2）を意識
    ↓
[Layer 2: 検出] commit → push → Codex 3パスレビュー（17項目×3回）→ PASS必須
    ↓
