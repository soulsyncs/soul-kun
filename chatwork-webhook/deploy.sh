#!/bin/bash
# =============================================================================
# chatwork-webhook デプロイスクリプト（Cloud Run版）
# =============================================================================
#
# 目的:
#   chatwork-webhook を安全にCloud Runにデプロイする
#
# 使い方:
...
#
# 推奨: scripts/safe_deploy.sh chatwork-webhook（煙テスト＋自動ロールバック付き）
#
...
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo -e "${BLUE}chatwork-webhook Cloud Run デプロイ${NC}"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo ""
...
if [ "$SKIP_TESTS" = true ]; then
    ...
    if ! python3 -m pytest tests/test_critical_functions.py tests/test_brain_type_safety.py -v --tb=short 2>&1 | tail -10; then
...
if [ "$DRY_RUN" = true ]; then
    echo "実行コマンド（参考）:"
    echo "  docker build -f chatwork-webhook/Dockerfile -t $IMAGE ."
...
docker build -f chatwork-webhook/Dockerfile -t "$IMAGE" .
docker push "$IMAGE"
echo -e "  ${GREEN}PASS${NC}"
echo ""
...
gcloud run deploy "$SERVICE" \
    ...
    --min-instances=1 \
    --max-instances=10 \
    --update-env-vars="USE_BRAIN_ARCHITECTURE=true,ENVIRONMENT=production,LOG_EXECUTION_ID=true,ENABLE_MEETING_TRANSCRIPTION=true,ENABLE_MEETING_MINUTES=true,ENABLE_IMAGE_ANALYSIS=true,MEETING_GCS_BUCKET=soulkun-meeting-recordings,TELEGRAM_CEO_CHAT_ID=8304510694,TELEGRAM_BOT_USERNAME=soulsyncs_bot" \
...
echo "  障害時:   scripts/rollback.sh chatwork-webhook"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
