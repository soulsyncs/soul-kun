...
│ 1. コード実装完了                                            │
│     ↓                                                       │
│ 2. CLAUDE.md「直近の主な成果」を更新（進捗報告）             │
│    ※ 進捗報告もPRに含め、Codexレビュー対象にする            │
│     ↓                                                       │
│ 3. git commit（コード＋進捗報告を一緒にコミット）            │
│     ↓                                                       │
│ 4. feature ブランチ作成 & push                               │
│     ↓                                                       │
│ 5. PR作成                                                   │
└─────────────────────────────────────────────────────────────┘
                              ↓
...
│                    レビューフェーズ                          │
├─────────────────────────────────────────────────────────────┤
│ 6. Codexレビューを待つ（gh pr checks で確認）                │
│     ↓                                                       │
│ 7. Codexレビュー結果を確認（gh pr view --comments）          │
│     ↓                                                       │
│ 8. レビュー結果をユーザーに報告                              │
│     ↓                                                       │
│ 9. 「マージしていいですか？」とユーザーに最終確認             │
│     ↓                                                       │
│ 10. ユーザーの承認後、マージ実行                             │
└─────────────────────────────────────────────────────────────┘
                              ↓
...
│     - DBマイグレーション: SQLスクリプト実行                  │
│     ↓                                                       │
│ 12. ユーザーに完了報告（全フロー完了を明示）                  │
└─────────────────────────────────────────────────────────────┘
```

**重要事項**:
- **ステップ2（進捗報告）をコミット前に実施**することで、Codexレビュー対象になる
- ステップ1〜8は自動で進めること
- ステップ9でユーザーの最終確認を必ず取ること（マージ前の確認）
- ステップ10〜12はユーザー承認後に連続して実行すること
- ステップ12で「全フロー完了」を明示的に報告すること
...
| 変更の種類 | CLAUDE.md | docs/CHANGELOG.md |
|-----------|-----------|-------------------|
| バグ修正（PATCH） | ✅ 必須 | ❌ 不要 |
| 機能追加（MINOR） | ✅ 必須 | ❌ 不要 |
| Phase完了（MAJOR） | ✅ 必須 | ✅ 必須 |
...
| 変更の種類 | バージョン変更 | 例 |
|-----------|---------------|-----|
| バグ修正 | PATCH +1 | v10.16.0 → v10.16.1 |
| 軽微な改善 | PATCH +1 | v10.16.1 → v10.16.2 |
| Codex指摘対応 | PATCH +1 | v10.16.2 → v10.16.3 |
| 新機能追加 | MINOR +1, PATCH=0 | v10.16.3 → v10.17.0 |
| Phase完了 | MINOR +1, PATCH=0 | v10.16.0 → v10.17.0 |
| 破壊的変更 | MAJOR +1 | v10.x.x → v11.0.0 |

### ■ 現在のバージョン確認方法

```bash
# 直近のバージョンを確認
git log --oneline -20 | grep -E "v[0-9]+\.[0-9]+\.[0-9]+"

# または CLAUDE.md の「直近の主な成果」を参照
```
...
```bash
# バグ修正の例
git commit -m "fix: Case-insensitive mention check (v10.16.2)"

# 新機能の例
git commit -m "feat: Add toall ignore feature (v10.16.0)"

# Phase完了の例
git commit -m "feat: Complete Phase 3.5 organization hierarchy (v10.17.0)"
```
...
対象ディレクトリ: `chatwork-webhook/`, `sync-chatwork-tasks/`, `remind-tasks/`
...
# 2. デプロイ実行
# 注意: 鉄則#4「APIは必ず認証必須」に従い、認証設定を適切に選択
gcloud functions deploy <function-name> \
  ...
  # 認証設定は用途に応じて選択:
  # - 内部API: --no-allow-unauthenticated（デフォルト、推奨）
  # - Webhook受信: --allow-unauthenticated + 署名検証をコード内で実装

# 3. デプロイ確認
gcloud functions describe <function-name> --region asia-northeast1
```

**デプロイが必要なケース**:
- `chatwork-webhook/main.py` を変更した場合 → `chatwork-webhook` をデプロイ
- `sync-chatwork-tasks/main.py` を変更した場合 → `sync-chatwork-tasks` をデプロイ
- `remind-tasks/main.py` を変更した場合 → `remind-tasks` をデプロイ
- `lib/` 配下を変更した場合 → **該当するすべてのCloud Functionsをデプロイ**

### ■ Cloud Run デプロイ（FastAPI）

対象ディレクトリ: `api/`

```bash
# 1. ビルド & デプロイ
# 注意: 鉄則#4「APIは必ず認証必須」に従い、IAM認証を有効化
cd api/
gcloud run deploy soulkun-api \
  --source . \
  --region asia-northeast1 \
  --no-allow-unauthenticated
  # IAM認証が有効化される。呼び出し側はIDトークンが必要

# 2. デプロイ確認
gcloud run services describe soulkun-api --region asia-northeast1
```
...
cloud_sql_proxy -instances=soulkun-production:asia-northeast1:soulkun-db=tcp:5432

# 2. マイグレーション実行
psql -h localhost -U soulkun_user -d soulkun_tasks -f migrations/<migration_file>.sql

# 3. 実行確認
psql -h localhost -U soulkun_user -d soulkun_tasks -c "\dt"
```
...
- バグ修正をした時
- テーブル定義を変更した時
- **進捗報告（CLAUDE.md更新）を行った時**

## 💬 コミットメッセージのルール

- 1行目: `{type}: {変更内容の要約} ({バージョン番号})`
  - type: `feat`（新機能）, `fix`（バグ修正）, `docs`（ドキュメント）, `refactor`（リファクタリング）
  - 例: `fix: Case-insensitive mention check (v10.16.2)`
- 2行目: 空行
- 3行目以降: 詳細な変更内容（日本語OK）
...
feat: Add toall ignore feature (v10.16.0)

オールメンション（[toall]）を無視する機能を追加。
- is_toall_mention() 関数を追加
- chatwork_webhook と check_reply_messages で呼び出し
- 27テストケース追加
...
マージ・デプロイ・進捗報告がすべて完了したら、以下の形式でユーザーに報告してください：
...
- **2026-01-23**: v10.16.4
  - 進捗報告フローの必須化（PR #23）
    - 実装完了後フローを12ステップ化（実装＋進捗報告→レビュー→デプロイ＋完了）
    - 進捗報告をコミット前（ステップ2）に移動し、Codexレビュー対象に
    - 進捗報告の記録ルール（CLAUDE.md vs CHANGELOG.md の棲み分け）を明文化
    - セマンティックバージョニングルールを定義
    - デプロイチェックリスト（認証設定を鉄則#4に準拠）を追加
    - 完了報告テンプレートを追加
...
- **2026-01-23**: v10.16.2
  - Codexレビューを経営者・PM・エンジニアの3視点で強化（PR #24）
    - 経営者視点：ビジネスインパクト、ランニングコスト分析、戦略整合性
    - PM視点：リスク管理、Phase整合性
    - エンジニア視点：既存の技術的チェック項目を維持
    - 追加自動化の提案セクション追加
