...
v10.40.12: 人生軸・価値観の検知パターン拡張
- 「人生軸は」「俺の軸は」など簡潔な表現に対応
- 「大事にしているのは」「大切にしていることは」追加
- 成功メッセージを統一
...
# 長期記憶を示すパターン
# v10.40.12: パターン拡張 - 人生軸・価値観の検知を強化
LONG_TERM_MEMORY_PATTERNS = [
    # 人生の軸・WHY（v10.40.12: 「人生軸」追加、一人称パターン強化）
    r"人生(の)?軸(は|を|が)?",  # 「人生軸は」「人生の軸は」両方対応
    r"(俺|私|僕|自分|わたし|おれ|ぼく)の(人生)?軸(は|を|が)?",  # 一人称 + 軸
    r"人生(の)?why",  # 「人生のWHY」「人生WHY」（小文字でマッチ）
    r"(俺|私|僕|自分|わたし|おれ|ぼく)の(人生の)?why",  # 一人称 + WHY
    r"生き方としての",
    r"存在意義",
    r"生きる(目的|理由|意味)",

    # 価値観・判断基準（v10.40.12: 「〜は」形式追加）
    r"(俺|私|僕|自分|わたし|おれ|ぼく)の?(判断基準|価値観|信条|信念)(は|を|が)?",
    r"判断基準(は|を|が)?",
    r"価値観(は|を|が)?",
    r"信条(は|を|が)?",
    r"信念(は|を|が)?",
    ...
    # 大切にしている表現（v10.40.12: 強化）
    r"大切にして(いる|いきたい)(こと|もの)?(は|を|が)?",
    r"大事にして(いる|いきたい)(こと|もの)?(は|を|が)?",
    r"大事にしているのは",
    r"大切にしていることは",
...
# 長期記憶保存成功メッセージ
# v10.40.12: シンプルで統一された返信文に変更
SUCCESS_MESSAGE = """大事な軸、ちゃんと覚えたウル！これからの会話に活かすウルね🐺✨
