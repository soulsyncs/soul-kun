...
def prepare_task_display_text(text: str, max_length: int = 40) -> str:
    ...
    try:
        ...
        # 3. 名前パターンを除去（★v10.17.1追加）
        # 「○○（読み仮名）さん」形式を除去（名前にスペースが含まれる場合も対応）
        # 例: "月宮 絵莉香（ツキミヤ エリカ）さん ありがとうございます！"
        text = re.sub(
            r'^.{1,30}[\(（][^\)）]+[\)）][\s　]*(さん|様|くん|ちゃん)[\s　]+',
            '', text
        )
        # シンプルな名前パターン: "田中さん " で始まる場合（本文が続く場合のみ）
        text = re.sub(r'^[^\s]{1,10}(さん|様|くん|ちゃん)[\s　]+(?=[ぁ-んァ-ンa-zA-Z])', '', text)
