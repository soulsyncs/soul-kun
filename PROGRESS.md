...
**Proactive Monitor 脳統合** 🔧 設計完了・実装待ち

| 項目 | 内容 |
|------|------|
| 問題 | Proactive Monitorが脳を経由せずメッセージを送信（CLAUDE.md鉄則違反） |
| 状態 | ドライランに戻した（proactive-monitor-00022-sul） |
| 設計書 | `docs/22_proactive_brain_integration.md` 作成完了 |
| 次のアクション | 脳統合を実装してから再有効化 |
...
**あるべき姿:**
```
トリガー検知 → 脳が状況理解 → 脳が判断 → 脳がメッセージ生成 → 送信
              （記憶参照）  （送るべきか）（自然な言葉で）
```
...
**Proactive Monitor 脳統合設計:** ✅ 設計書完成
> CLAUDE.md鉄則違反を発見し、ドライランに戻して設計書を作成した。
>
> **問題の発見:**
> - Proactive Monitorが脳を経由せずメッセージを生成・送信していた
> - 鉄則3「脳が判断、機能は実行」に違反
> - 鉄則2「脳は全記憶にアクセス」を活用していなかった
>
> **対応:**
> 1. 本番有効化（00021-xev）→ ドライラン（00022-sul）に戻した
> 2. 脳統合設計書を作成: `docs/22_proactive_brain_integration.md`
>
> **設計のポイント:**
> - ProactiveMonitor = トリガー検知 + 送信実行のみ（機能）
> - SoulkunBrain = 記憶参照 + 状況理解 + 判断 + メッセージ生成（脳）
> - 新メソッド: `SoulkunBrain.generate_proactive_message()`
>
> **次のアクション:**
> 1. models.pyにProactiveMessageResultを追加
> 2. core.pyにgenerate_proactive_message()を実装
> 3. proactive.pyを脳経由に修正
> 4. テスト作成・実行
> 5. 脳統合版で再有効化
