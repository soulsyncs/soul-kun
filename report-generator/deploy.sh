#!/bin/bash
# =============================================================================
# report-generator デプロイスクリプト（Cloud Run版）
# =============================================================================
#
# 目的:
#   report-generator を安全にCloud Runにデプロイする
#
# 使い方:
#   ./report-generator/deploy.sh              # 本番デプロイ
#   ./report-generator/deploy.sh --dry-run    # 確認のみ（デプロイしない）
#   ./report-generator/deploy.sh --skip-tests # テストをスキップ
#
# =============================================================================
...
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo -e "${BLUE}report-generator Cloud Run デプロイ${NC}"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo ""
...
if [ "$SKIP_TESTS" = true ]; then
    ...
    if ! python3 -c "import sys; sys.path.insert(0, 'report-generator'); from main import app; print('OK')"; then
...
if [ "$DRY_RUN" = true ]; then
    echo "実行コマンド（参考）:"
    echo "  docker build -f report-generator/Dockerfile -t $IMAGE ."
...
docker build -f report-generator/Dockerfile -t "$IMAGE" .
docker push "$IMAGE"
echo -e "  ${GREEN}PASS${NC}"
echo ""
