#!/bin/bash
# =============================================================================
# cleanup-old-data デプロイスクリプト（Cloud Run版）
# =============================================================================
#
# 目的:
#   cleanup-old-data を安全にCloud Runにデプロイする
#
# 使い方:
#   ./cleanup-old-data/deploy.sh              # 本番デプロイ
#   ./cleanup-old-data/deploy.sh --dry-run    # 確認のみ（デプロイしない）
#   ./cleanup-old-data/deploy.sh --skip-tests # テストをスキップ
#
# =============================================================================
...
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo -e "${BLUE}cleanup-old-data Cloud Run デプロイ${NC}"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo ""
...
if [ "$SKIP_TESTS" = true ]; then
    ...
    if ! python3 -c "import sys; sys.path.insert(0, 'cleanup-old-data'); from main import app; print('OK')"; then
...
if [ "$DRY_RUN" = true ]; then
    echo "実行コマンド（参考）:"
    echo "  docker build -f cleanup-old-data/Dockerfile -t $IMAGE ."
...
docker build -f cleanup-old-data/Dockerfile -t "$IMAGE" .
docker push "$IMAGE"
echo -e "  ${GREEN}PASS${NC}"
echo ""
