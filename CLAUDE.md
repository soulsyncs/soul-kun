...
4. PR作成
    ↓
5. Codexレビュー完了を待つ（gh run list で確認）
    ↓
...
```bash
# ステップ1: ワークフロー完了を確認（completedになるまで待つ）
...
**注意**: `gh pr checks`が「no checks reported」を返す場合は、`gh run list`でワークフロー完了を確認してください。
