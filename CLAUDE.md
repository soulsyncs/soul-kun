...
### 1-1. 正当な例外：管理ダッシュボード【v11.2.0更新】

管理ダッシュボードは「ソウルくんの脳みそを可視化・直接操作するツール」であり、Brain経由を必要としない。

#### 【読み取り操作】以下の**全条件**を満たす場合に限り、Brainを経由せずDBから直接取得してよい：
...
#### 【書き込み操作】以下の**全条件**を満たす場合に限り、Brainを経由せずDBを直接変更してよい：

| 条件 | 説明 |
|------|------|
| **管理者限定** | Level 6（代表/CFO）のみ、または Level 5以上（操作内容による） |
| **Brain状態の変更** | ナレッジ・メンバー・教え・設定等の「脳の状態」を変更する操作 |
| **判断なし** | AI による自動判断を介さず、管理者が意図的に実行する操作 |
| **能動的出力なし** | 変更の結果としてChatWork等への送信・通知を自動発生させない |
| **監査ログ記録** | 書き込み操作は全て監査ログに記録すること（CLAUDE.md §3 鉄則#3） |

**根拠**: 管理ダッシュボードはBrainの「観測・直接操作ツール」。ChatWork/LINEなどのメッセンジャー経由の全入出力は引き続き必ずBrainを通ること（この例外はダッシュボードにのみ適用）。

**該当する機能**: 管理ダッシュボード（`docs/32_admin_dashboard.md`）

> 制定理由（v11.2.0）: メンバー登録・CEOの教え追加など管理画面からの直接操作は「脳の状態を変更する行為」であり、Brainを経由する意味がない。Brainを経由させると不必要なレイテンシとコストが発生し、操作の明確な意図も失われる。3AI全員一致（2026-02-21）。
