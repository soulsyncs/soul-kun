...
@pytest.fixture
def mock_pool():
    """Async/sync database pool mock with context manager support.

    pool.connect() returns a dual context manager that supports both:
    - async with pool.connect() as conn: (legacy async path)
    - with pool.connect() as conn: (new asyncio.to_thread sync path)
    conn.execute supports both sync Mock and async AsyncMock patterns.
    """
    pool = MagicMock()

    # sync conn for asyncio.to_thread() calls
    sync_conn = Mock()
    sync_conn.commit = Mock()
    sync_conn.rollback = Mock()
    sync_conn.execute = Mock(return_value=_make_db_result())

    # async conn for legacy async paths
    ...
    pool.connect = _DualCtx
    pool._conn = sync_conn  # expose for test assertions (sync is primary now)
    pool._async_conn = async_conn  # expose async conn for tests that need it
    return pool
...
class TestConnectWithOrgContext:
    ...
    @pytest.mark.asyncio
    async def test_sets_and_resets_org_context_sync(self, monitor, mock_pool):
        """sync版 _connect_with_org_context_sync のテスト（asyncio.to_thread用）"""
        conn = mock_pool._conn
