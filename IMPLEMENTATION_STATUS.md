...
**原因分析:**
- `search_tasks_from_db()` 関数（chatwork-webhook/main.py:1525行）
- `WHERE room_id = :room_id` でフィルタリングしている
- つまり「質問したチャットルームのタスクしか検索していない」
- 他のチャットルームにあるタスクは検索対象外

**例:**
```
カズさんが「管理部チャット」で「自分のタスクを教えて」と質問
→ ソウルくんは「管理部チャット」のタスクだけ検索
→ カズさんのタスクは「営業チャット」や「プロジェクトA」にある
→ 結果: 「見つからない」と返答
```

**改善方法:**
1. `search_tasks_from_db()` に `search_all_rooms` オプションを追加
2. 「自分のタスク」の場合は `room_id` フィルタを外す
3. 結果にルーム名を表示（どのチャットのタスクか分かるように）

**影響範囲:**
- chatwork-webhook/main.py: `search_tasks_from_db()`, `handle_chatwork_task_search()`
- 工数目安: 2-3時間

**対応予定:** Phase 1-B完了後（優先度は中）
