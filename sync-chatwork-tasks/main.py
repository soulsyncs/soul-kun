    ...
    for task_id, body in tasks:
        try:
            ...
            cursor.execute("""
                ...
                WHERE task_id = %s
                  AND organization_id = %s
            """, (summary, task_id, _ORGANIZATION_ID))
            conn.commit()
            result["success"] += 1
...
def regenerate_bad_summaries(
    ...
    Returns:
        処理結果の辞書
    """
    # v10.14.1: 全件数を取得（v10.14.3: organization_idフィルタ復活）
    cursor.execute("""
        SELECT COUNT(*) FROM chatwork_tasks
    ...
    # v10.14.1: offsetベースでバッチ取得（v10.14.3: organization_idフィルタ復活）
    cursor.execute("""
    ...
    for task_id, body, current_summary in tasks:
        try:
            ...
            if new_summary and validate_summary(new_summary, body):
                ...
                # v10.14.3: organization_idフィルタ復活
                cursor.execute("""
                    UPDATE chatwork_tasks
                    SET summary = %s
                    WHERE task_id = %s
                      AND organization_id = %s
                """, (new_summary, task_id, _ORGANIZATION_ID))
                conn.commit()
...
def report_summary_quality(
    ...
    # 1. 全体統計（v10.14.1: organization_idでフィルタ、v10.14.2: NULL対応、v10.14.3: フィルタ復活）
    cursor.execute("""
        SELECT
            COUNT(*) AS total_open,
    ...
    # 2. 低品質要約をサンプリングチェック（最新50件）（v10.14.1: organization_idでフィルタ復活）
    cursor.execute("""
    ...
    with pool.connect() as conn:
        ...
        result = conn.execute(
            ...
            """),
            {
                ...
                "pattern2": f"%{_escape_ilike(partial_name)}%",
...
def get_chatwork_account_id_by_name(name):
    ...
    with pool.connect() as conn:
        ...
        result = conn.execute(
            ...
            {"pattern": f"%{_escape_ilike(clean_name)}%"}
        ).fetchone()
        if result:
        ...
        result = conn.execute(
            sqlalchemy.text("""
                ...
                WHERE REPLACE(REPLACE(name, ' ', ''), '　', '') ILIKE :pattern ESCAPE '\\'
                LIMIT 1
            """),
            {"pattern": f"%{_escape_ilike(normalized_name)}%"}
        ).fetchone()
        if result:
...
def search_tasks_from_db(room_id, assigned_to_account_id=None, assigned_by_account_id=None, status="open"):
    ...
    try:
        ...
        with pool.connect() as conn:
            ...
            """
            params = {"room_id": room_id, "org_id": _ORGANIZATION_ID}
    ...
    try:
        ...
        with pool.begin() as conn:
            conn.execute(
                ...
                """),
                {"task_id": task_id, "status": status, "org_id": _ORGANIZATION_ID}
            )
...
def process_overdue_tasks():
    ...
    try:
        ...
        with pool.connect() as conn:
            ...
            """), {"today_timestamp": int(datetime.combine(today, datetime.min.time()).replace(tzinfo=JST).timestamp()), "org_id": _ORGANIZATION_ID})
    ...
    try:
        ...
        for room in rooms:
            ...
            for task in open_tasks:
                ...
                cursor.execute("""
                    SELECT task_id, status, limit_time, assigned_by_name, body, summary FROM chatwork_tasks WHERE task_id = %s AND organization_id = %s
                ...
                if existing:
                    ...
                    cursor.execute("""
                        ...
                        WHERE task_id = %s
                          AND organization_id = %s
                    """, (body, limit_datetime, room_name, assigned_to_name, new_summary, task_id, _ORGANIZATION_ID))
                    # ★★★ v10.4.0: UPDATEをコミット ★★★
                    conn.commit()
                else:
                    # 新規タスクの挿入
