...
**バージョン:** v1.3.0
**作成日:** 2026-01-30
**最終更新:** 2026-01-30（Doc Contract追加、日常運用導線追加）
...
| **書くこと** | 6層アーキテクチャ詳細、LLM Brain憲法、Tool定義、System Prompt、移行計画 |
| **書かないこと** | 原則の要点（→CLAUDE.md）、DB実装詳細（→03章）、API実装詳細（→04章） |
| **SoT（この文書が正）** | LLM Brain憲法、6層アーキテクチャ、Tool定義、System Prompt v2.0、移行計画 |
| **Owner** | カズさん（代表） |
...
### 15.4 Authorization Gate（権限チェック層）
