...
| 項目 | 内容 |
|------|------|
| **この文書の役割** | Phase 3ナレッジ検索機能の試験運用手順書・デプロイガイド |
| **書くこと** | デプロイ前準備、デプロイ手順、検索APIの使い方、試験運用フェーズの手順、トラブルシューティング、設定項目一覧 |
| **書かないこと** | 設計の詳細・根拠（→05_phase3_knowledge_detailed_design.md）、Googleドライブ連携の設計（→06_phase3_google_drive_integration.md） |
| **SoT（この文書が正）** | 試験運用のフェーズ分け、展開基準（スコア閾値、回答拒否率、フィードバック率）、環境変数設定値、チューニング推奨値 |
| **Owner** | Tech Lead |
| **更新トリガー** | デプロイ手順の変更、試験運用フェーズの変更、設定値の変更 |
