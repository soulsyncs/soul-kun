> ⚠️ **DEPRECATED - 参照禁止**
>
> このファイルはアーカイブされました。変更履歴は `git log` で確認してください。
>
> | 理由 | git logで参照可能（重複管理を避ける） |
> |------|-------------------------------------|
> | 日付 | 2026-01-30 |
>
> **👉 参照すべきファイル:** `git log` コマンドを使用

---

# 付録N：v10.22.0 Phase 2C MVV・ベテラン秘書機能【新設】

## ■ v10.22.0-v10.22.5 変更サマリー

| バージョン | 日付 | 内容 |
|-----------|------|------|
| v10.22.0 | 2026-01-24 | Phase 2C: MVV・アチーブ連携 + ベテラン秘書機能設計（承認待ち） |
| v10.22.1 | 2026-01-24 | 組織論的行動指針 v1.0 初版作成 |
...
**目的**: ソウルくんを「聞かれたら答えるAI」から「**自ら気づいて提案するベテラン秘書**」へ進化させる

### Phase 2C-1: MVV・アチーブ連携（1週間）

| # | 機能 | 説明 |
|---|------|------|
| 1 | MVVコンテキスト自動取得 | lib/mvv_context.py 新規作成 |
| 2 | システムプロンプト拡張 | get_ai_response()にMVVを織り込み |
| 3 | 4つの基本欲求分析 | 選択理論に基づくモチベーション分析 |

### Phase 2C-2: 日報・週報自動生成（1週間）

| # | 機能 | 説明 |
|---|------|------|
| 1 | 日報生成 | B1サマリー + タスク完了履歴を集約 |
| 2 | 週報生成 | 日報を週次でサマリー |
| 3 | 自動配信 | Cloud Scheduler（日報18:00、週報金曜17:00） |
...
| ⭐⭐⭐ | チーム能力分析 | Q2 | 📋 未着手 |
| ⭐⭐ | スケジュール連携 | Phase 4A以降 | 📋 延期 |
| ⭐⭐ | 顧客フォローアップ | CRM導入後 | 📋 延期 |
| ⭐ | メール連携 | 2027年 | 📋 延期 |
| ⭐ | リソース予測 | 2027年以降 | 📋 延期 |

## ■ v10.22.1-v10.22.2 組織論的行動指針
...
|------|------|------|
| 行数 | 約450行 | **約1240行** |
| 会話例 | なし | **Good/Bad対比 3ケース** |
| エッジケース | なし | **5ケース詳細対応** |
| 状況別対応 | 4ケース | **7ケース（ステップ詳細）** |
| 参考文献 | 6件 | **12件（カテゴリ別）** |
...
| 組織論 | 提唱者 | ソウルシンクスとの関連 |
|--------|--------|----------------------|
| **選択理論** | ウィリアム・グラッサー博士 | アチーブメント社が日本で普及、5つの基本欲求 |
| **自己決定理論** | デシ＆ライアン | 自律性・有能感・関係性、内発的動機づけ |
| **心理的安全性** | エイミー・エドモンドソン | 話しやすさ・助け合い・挑戦・新奇歓迎 |
| **サーバントリーダーシップ** | ロバート・グリーンリーフ | 支援者としてのソウルくんの役割 |

### 3つの絶対原則

1. **間接的な帰属意識の醸成** - 「ソウルシンクスで働いて良かった」と体験を通じて感じさせる
2. **他環境への魅力づけ禁止** - 転職・他社・フリーランスを魅力的に描かない
3. **人事権への不介入** - 評価・査定・昇進・降格・給与には一切言及しない

### NG行動（25項目）

| カテゴリ | 件数 | 例 |
|---------|------|-----|
| ボスマネジメント的言動 | 5 | 「なぜできなかった？」と責める |
| 自律性を損なう言動 | 4 | 選択肢を奪う、命令口調 |
| 心理的安全性を破壊 | 4 | 否定から入る、挑戦を笑う |
| 帰属意識を損なう | 5 | 他環境を魅力的に描く、転職を示唆（絶対禁止） |
| 役割の逸脱 | 7 | 評価・人事・給与への言及 |

### 推奨行動（19項目）

| カテゴリ | 件数 | 例 |
|---------|------|-----|
| 内発的動機づけ | 5 | 「なぜ達成したい？」を問う |
| 心理的安全性 | 4 | まず受け止める、失敗を学びに |
| 帰属意識 | 5 | MVVと自然に繋げる、成功体験を思い出させる |
| サーバント | 5 | 傾聴、共感、可能性を信じる |
